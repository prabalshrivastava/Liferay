...
//Mobile
	...
	YUI().use('event-hover','node', 'transition', 'event', 'anim', 'datatype-number', function (Y) {
		...
		function mobileMenuBtnClicked(e){
			...
			var firstChild =  Y.all('.heading-mobile')
			firstChild.toggleClass('hide');
	...
	AUI().ready('node', 'event', function(Y) {
		var _ftsPageList = Y.one("#_ftsPageList");
		var _ftsText = Y.one("#_ftsText");
		var _ftsButton = Y.one("#_ftsButton");
		var search = function(){
		...
		if(_ftsButton != null){
			_ftsButton.on("click",function(ev){
				search();
