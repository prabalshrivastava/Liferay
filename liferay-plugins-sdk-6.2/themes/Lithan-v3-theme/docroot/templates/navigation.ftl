	...
	<ul aria-label="<@liferay.language key="site-pages" />" role="menubar" class="main-navigation-list font-none dl-menu">
		...
		<#list nav_items as nav_item>
			...
			<#if nav_item.getName() == 'Products'>
				 <#if (samUtil.hasProductAccess(themeDisplay.getScopeGroupId(),themeDisplay.getUserId()))>
					<#assign show_nav_item = "true" />
				<#else>
					<#assign show_nav_item = "false" />	
				</#if>	
			</#if>
										...
										<#if nav_child.hasChildren()>
											<#list nav_child.getChildren() as nav_child_child>
												<#if (samUtil.getExpandoValueString(nav_child_child.getLayout().getExpandoBridge(), "Header", themeDisplay) == "true")>
													<#assign childChildNav = "true" />
												<#else>
													<#assign childChildNav = "false" />
												</#if>
											</#list>
												<#if (childChildNav == "true")>
													<img alt="Menu Drop Down" src="${theme_display.getPathThemeImages()}/menu/menuDropDownIcon1.png" class="resizeImage"/>
												</#if>
										</#if>
									</a>
