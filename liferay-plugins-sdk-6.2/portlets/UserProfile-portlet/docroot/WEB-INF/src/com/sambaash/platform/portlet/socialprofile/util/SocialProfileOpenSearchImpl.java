...
public class SocialProfileOpenSearchImpl extends HitsOpenSearchImpl {

	public static final String TITLE = "UserProfile Search: ";

	@Override
	public Indexer getIndexer() {
		return IndexerRegistryUtil.getIndexer(SocialProfile.class);
	}

	@Override
	public String getPortletId() {
		return SocialProfileIndexer.PORTLET_ID;
	}
	...
	@Override
	public String search(HttpServletRequest request, long groupId, long userId,
		...
		try {
			...
			Object[] values = addSearchResults(queryTerms, keywords, startPage,
					itemsPerPage, total, start, getTitle(keywords),
					getSearchPath(), format, themeDisplay);
			...
			for (int i = 0; i < results.getDocs().length; i++) {
				...
				long resultGroupId = GetterUtil.getLong(result
						.get(Field.GROUP_ID));
				...
				long resultScopeGroupId = GetterUtil.getLong(result
				...
				Summary summary = getSummary(indexer, result,
						themeDisplay.getLocale(), snippet, portletURL);

				String title = summary.getTitle();
				String url = getURL(themeDisplay.getCompanyId(), groupId,
						result.get("screenName"));

				if (Validator.isNull(url)) {
					url = getURL(themeDisplay, resultScopeGroupId, result,
							portletURL);
				}
				...
				long entryClassPK = GetterUtil.getLong(result
				...
				addSearchResult(root, resultGroupId, resultScopeGroupId,
						entryClassName, entryClassPK, title, url, modifiedDate,
						content, tags, ratings, score, format);
			}
	 ...
	 * @param groupId
	 * @param userId
	 * @return User profile public URL
	 */
	private String getURL(long companyId, long groupId, String screenName) {
		String url = "";

		if (Validator.isNotNull(screenName)) {
			url = SambaashUtil.getPortalURL(companyId, groupId);
			url += StringPool.FORWARD_SLASH + screenName;
		}

		return url;
	}

	public static final String SEARCH_PATH = "/c/userprofile/open_search";

	private static Log _log = LogFactoryUtil
			.getLog(SocialProfileOpenSearchImpl.class);
