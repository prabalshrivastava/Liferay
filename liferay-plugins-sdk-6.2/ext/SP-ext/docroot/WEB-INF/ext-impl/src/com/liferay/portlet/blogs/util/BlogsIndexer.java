...
public class BlogsIndexer extends BaseIndexer {
	...
	protected Document doGetDocument(Object obj) throws Exception {
		...
		document.addText(Field.DESCRIPTION, entry.getContent());
		document.addDate(Field.MODIFIED_DATE, entry.getModifiedDate());
		document.addDate("displayDate", entry.getDisplayDate());
		document.addText(Field.TITLE, entry.getTitle());
		
		// Added by Clark
		document.addKeyword("urlTitle", entry.getUrlTitle());
		// End
		...
		try {
			...
			if (Validator.isNotNull(entry.getExpandoBridge().getAttribute(AUTHOR))) {
				int type = entry.getExpandoBridge().getAttributeType(AUTHOR);
				if (type == ExpandoColumnConstants.STRING) {
					Serializable authorName = entry.getExpandoBridge().getAttribute(AUTHOR);
		...
		if (Validator.isNull(author)) {
			author = HtmlUtil.escape(PortalUtil.getUserName(entry.getUserId(), entry.getUserName()));
		}
	...
	private static final String AUTHOR = "External-Author";
}