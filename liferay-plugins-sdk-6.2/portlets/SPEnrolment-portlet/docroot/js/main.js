...
var newExamFeesAmount = 0;
var newLocalFeesAmount = 0;
...
function renderFeeStructureTableData(data) {
    ...
    data[0].resutlBySubject.forEach(function(fee) {
    	if(fee.priceSubType=="LocalFees") {
    		newLocalFeesAmount = newLocalFeesAmount + fee.amountByBaseCurrency;
    	} else if(fee.priceSubType=="ExamFees") {
    		newExamFeesAmount = newExamFeesAmount + fee.amountByBaseCurrency;
    	}
                    ...
                    if (_.isEmpty(data)) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                        	if (data[i].PricingType == "MiscFees") {
                                ...
                                getMiscFeesData(enrolment, data[i], false);
                            } else {
...
function backToListing() {
	...
	try {
		if (enrolmentIdsKey) {
			if (sessionStorage) {
				sessionStorage.removeItem(enrolmentIdsKey);
			} else {
				deleteCookie(enrolmentIdsKey);
			}
		}
	} catch(e) {
		// NOP
...
function getMiscFeesData(enrolment, priceScheme, populate) {    	
	var miscFeesInput = {
			scopeGroupId: _SCOPE_GROUP_ID,
		    programmeScheduleCode: encodeURI(enrolment.programSemester.storageId),
		    scheduleCode: encodeURI(enrolment.programSemester.ScheduleCode),
	...
	ajaxCall(
	        ...
	        function () {
	        	...
	        	if(data.length > 0){
	  		    	...
	  		    	if((typeof populate === "undefined") || populate==true) {
