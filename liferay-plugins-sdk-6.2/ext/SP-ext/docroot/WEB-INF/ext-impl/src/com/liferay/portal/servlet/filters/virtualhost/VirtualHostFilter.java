...
import com.sambaash.platform.util.SambaashExtUtil;
...
public class VirtualHostFilter extends BasePortalFilter {
	...
	protected void processFilter(
		...
		//Added for user profile friendly url
		String orgFriendlyURL = friendlyURL;
		//till here
		...
		/*
		 * SPD-465 Adding feature for adding pages per country
		 */
		if(Validator.isNotNull(friendlyURL)){
			friendlyURL = SambaashExtUtil.updateFriendlyUrl(request, friendlyURL, companyId, layoutSet.getGroupId(), i18nLanguageId);
		}
		...
		try {
			...
			// Added for user profile friendly url
			/**try {
				String friendlyUrl = (orgFriendlyURL
				.startsWith(StringPool.FORWARD_SLASH) && orgFriendlyURL
				.length() > 1) ? orgFriendlyURL
				.substring(1) : orgFriendlyURL;
				
				UserLocalServiceUtil.getUserByScreenName(companyId, friendlyUrl);
								
				forwardURL.append("/profile");
				request.setAttribute(SambaashConstants.PROFILE_WITH_SCREEN_NAME, friendlyUrl);
			...
			try {
				...
				if (_log.isDebugEnabled()) {
					_log.debug(
							"Public Profile Page  : "
									+ SambaashUtil.getParameter(SambaashConstants.URL.PROFILE_IND_PUBLIC,
											layoutSet.getGroupId())
									+ " : layoutSet.getGroupId() : " + layoutSet.getGroupId());
				}

				forwardURL.append(
						SambaashUtil.getParameter(SambaashConstants.URL.PROFILE_IND_PUBLIC, layoutSet.getGroupId()));
				request.setAttribute(SambaashConstants.PROFILE_WITH_SCREEN_NAME, friendlyUrl);
