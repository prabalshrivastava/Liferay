...
<portlet:actionURL name="updateGlobal" var="updateGlobalURL">
	<portlet:param name="tabs1" value="global" />
</portlet:actionURL>

<aui:form action="<%= updateGlobalURL %>">
	...
	<aui:fieldset>
		...
		>
			...
			<%
			String samlRole = SamlUtil.getRole(companyId);
			%>

			<aui:option label="identity-provider" selected='<%= "idp".equals(samlRole) %>' value="idp" />
			<aui:option label="service-provider" selected='<%= "sp".equals(samlRole) %>' value="sp" />
		</aui:select>
...
<c:if test="<%= Validator.isNotNull(entityId) %>">
	<aui:fieldset label="certificate-and-private-key">
		...
		<aui:form action="<%= updateCertificateURL %>">
			<div 
				class="<%= ((x509Certificate == null) && Validator.isNotNull(MetadataManagerUtil.getLocalEntityId(companyId, groupId))) ? "" : "aui-helper-hidden hide" %>" 
...
<%!
private static Log _log = LogFactoryUtil.getLog("admin.global_jsp");
%>
