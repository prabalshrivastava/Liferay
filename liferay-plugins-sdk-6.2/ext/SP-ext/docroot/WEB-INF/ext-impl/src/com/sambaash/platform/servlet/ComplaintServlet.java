...
public class ComplaintServlet extends HttpServlet {
	...
	private static final long serialVersionUID = 5805287278315501208L;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if ("SubscriptionConfirmation".equalsIgnoreCase(sns.getType())) {
				...
				if ("Complaint".equalsIgnoreCase(message.getNotificationType())) {
					SPMailBlackListLocalServiceUtil.addComplaint(sns.getMessage(), message);
					_log.debug("Received Complaint message : " + message.toString());
				} else {
	...
	private static Log _log = LogFactoryUtil.getLog(ComplaintServlet.class);
