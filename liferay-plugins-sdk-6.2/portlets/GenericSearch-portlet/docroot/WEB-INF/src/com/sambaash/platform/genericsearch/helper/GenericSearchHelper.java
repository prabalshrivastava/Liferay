...
public class GenericSearchHelper {
	...
	public GenericSearchConfig parseConfiguration(PortletPreferences preferences ) {
		...
		config.setItemsPerPage(GetterUtil.getInteger(preferences.getValue(
				GenericSearchConstants.PREF_ITEMS_PER_PAGE, "3"), 3));
		config.setTitleForResultsList(GetterUtil.getString(preferences.getValue(
				GenericSearchConstants.RESULT_TITLE, ""), ""));
		config.setLoadMoreText(GetterUtil.getString(preferences.getValue(
				GenericSearchConstants.LOAD_MORE_TEXT, "More records"), "More records"));
		config.setEnabledComponentFilters(GetterUtil.getBoolean(preferences
				.getValue(GenericSearchConstants.PREF_COMP_FILTERING, "Off"),
				false));
		config.setResultsOrientation(preferences.getValue(
				GenericSearchConstants.PREF_RESULTS_ORIENTATION, "Horizontal"));
		config.setHideFitlers(GetterUtil.getBoolean(preferences.getValue(
				GenericSearchConstants.PREF_HIDE_FILTERS, "Off")));
		config.setDateFitlers(GetterUtil.getBoolean(preferences.getValue(
				GenericSearchConstants.PREF_DATE_FILTERING, "Off")));
		config.setDateFitlersField(GetterUtil.getString(preferences.getValue(
				GenericSearchConstants.PREF_DATE_FILTERING_FIELD, Field.CREATE_DATE)));
		config.setFreeTextSearch(GetterUtil.getString(preferences.getValue(
				GenericSearchConstants.PREF_FREE_TEXT_SEARCH, GenericSearchConstants.PARTIAL_MATCH)));
		config.setHideExports(GetterUtil.getBoolean(preferences.getValue(
				GenericSearchConstants.PREF_HIDE_EXPORTS, "Off")));
		config.setAdminOnlyExports(GetterUtil.getBoolean(preferences.getValue(
				GenericSearchConstants.PREF_ADMIN_EXPORTS, "Off")));
		config.setDisableUnwantedFilters(GetterUtil.getBoolean(preferences
				.getValue(GenericSearchConstants.PREF_DISABLE_UNWANTED_FILTERS,
						"Off")));
		config.setDisableTextSearch(GetterUtil.getBoolean(preferences.getValue(
				GenericSearchConstants.PREF_DISABLE_TEXT_SEARCH, "Off")));
		config.setFilterTopLabel(preferences.getValue(
				GenericSearchConstants.PREF_FILTER_LABEL,
				GenericSearchConstants.DEFAULT_FILTER_LABEL));
		config.setLeftFilterPlacement(GetterUtil.getBoolean(preferences
				.getValue(GenericSearchConstants.PREF_FILTER_PLACEMENT, "On")));
		config.setClearFiltersOnTextSearch(GetterUtil.getBoolean(preferences
				.getValue(GenericSearchConstants.PREF_CLEAR_FILTERS_TEXTSEARCH,
						"Off")));
		config.setHideFavourites(GetterUtil.getBoolean(preferences
				.getValue(GenericSearchConstants.PREF_HIDE_FAVOURITES,
						"Off")));
		config.setSortFitlers(GetterUtil.getBoolean(preferences.getValue(
				GenericSearchConstants.PREF_SORT_FILTERING, "Off")));
		config.setFavouritesRoles(GetterUtil.getString(preferences.getValue(
		...
		for (String key : preferences.getMap().keySet()) {
			...
			String count = preferences
					.getValue("filter--count--" + rowID, null);
			String subType = preferences
					.getValue("filter--selection--" + rowID, null);
			String vocab = preferences
					.getValue("filter--vocab--" + rowID, null);
			String fieldVocabulary = preferences
					.getValue("filter--fieldVocabulary--" + rowID, null);
			String field = preferences
					.getValue("filter--field--" + rowID, null);
			String text = preferences
					.getValue("filter--config--" + rowID, null);
			String apiList = preferences
					.getValue("filter--apiList--" + rowID, null);
			String display = preferences.getValue("filter--display--" + rowID,
					null);
			String opr = preferences.getValue("filter--opr--" + rowID, null);
			String values = preferences.getValue("filter--values--" + rowID,
					null);
			int level = GetterUtil.getInteger(preferences.getValue("filter--level--" + rowID,
					null));
			int pos = GetterUtil.getInteger(
					preferences.getValue("filter--position--" + rowID, null),
					-1);
			int filterId = GetterUtil.getInteger(
					preferences.getValue("filter--filterId--" + rowID, null)	);
			...
			filter.setLabel(preferences.getValue("filter--label--" + rowID,
					null));
	...
	public static Map<String, String> getFilterOptionsMap(
			GenericSearchFilter filter, ThemeDisplay themeDisplay) {
		try {
			Map<String, String> options = new LinkedHashMap<String, String>();
			if (GenericSearchFilter.TYPE_TAG.equals(filter.getType())
					|| ((GenericSearchFilter.TYPE_FIELD.equals(filter.getType())) && (!GenericSearchFilter.SELECT_API.equals(filter.getDisplayCount())) 
				...
				if ((filter.getLevel() == 0) || (Validator.isNull(filter.getLevel()))){
					categories = AssetCategoryLocalServiceUtil
							.getVocabularyCategories(filter.getFieldVocabularyId(), -1, -1,
									null);
				} else if (filter.getLevel() == 1) {
					categories = AssetCategoryLocalServiceUtil
							.getVocabularyCategories(0, filter.getFieldVocabularyId(), -1, -1,
									null);
				...
				for (AssetCategory assetCategory : categoriesSorted) {
					options.put(String.valueOf(assetCategory.getCategoryId()),
				...
				if ((filter.getLevel() == 0) || (Validator.isNull(filter.getLevel()))){
					categories = AssetCategoryLocalServiceUtil
							.getVocabularyCategories(filter.getVocabId(), -1, -1,
									null);
				} else if (filter.getLevel() == 1) {
					categories = AssetCategoryLocalServiceUtil
							.getVocabularyCategories(0, filter.getVocabId(), -1, -1,
									null);
				} else if (filter.getLevel() == 2) {
					List<AssetCategory> categoriesTemp = AssetCategoryLocalServiceUtil
							.getVocabularyCategories(filter.getVocabId(), -1, -1,
	...
	public static Map<String, Object> getFilterDataMap(
	...
	public List<Document> search(GSFavourite favourite,int start, int end) throws Exception {
		...
		GenericSearchHelper.GSSearchItems searchItems = null;//new GenericSearchHelper.GSSearchItems(searchItemsJson);
		return search(favourite.getCompanyId(), favourite.getGroupId(), start, end, config, searchItems);
	}
	public List<Document> search(long companyId,long scropeGroupId,int start, int end,GenericSearchConfig config,GSSearchItems searchItems) throws Exception {
		...
		if (Validator.isNotNull(searchQuery)) {
			Hits hits = SearchEngineUtil.search(searchContext.getSearchEngineId(), companyId,
					searchQuery, sort, start, end);
	...
	public JSONArray searchEMS(PortletRequest req,long companyId,long scropeGroupId,int start, int end,GenericSearchConfig config,GSSearchItems searchItems) throws Exception {
		...
		try {
				...
				String page = String.valueOf( start / limit);
				String conditions = "";
				JSONArray filters = JSONFactoryUtil.createJSONArray("[{'contentJson.Status':['Active']}]");
		...
		//response = "{\"content\":[{\"createdBy\":\"20199\",\"createdDate\":\"2018-10-17T16:33:30.000+0000\",\"lastModifiedBy\":\"20199\",\"lastModifiedDate\":\"2018-10-17T16:33:30.000+0000\",\"modelId\":3,\"storageId\":\"20199\",\"siteId\":\"21424\",\"contentJson\":{\"Status\":\"Active\",\"DateOfBirth\":\"\",\"IDNumber2\":\"34534535\",\"ContactNumberSingapore\":\"45646454564\",\"FirstName\":\"biswojit\",\"Gender\":\"Male\",\"Salutation\":\"Mr\",\"IDType\":\"{\\\"id\\\":\\\"FIN\\\",\\\"name\\\":\\\"FIN\\\"}\",\"PrimaryEmailAddress\":\"\",\"SecondaryEmailAddress\":\"\",\"IDType2\":\"{\\\"id\\\":\\\"NRIC\\\",\\\"name\\\":\\\"NRIC\\\"}\",\"UserId\":\"20199\",\"FullName\":\"dfsdfsdf\",\"LastName\":\"mohapatra\",\"ContactNumberOthers\":\"45646456456\",\"IDNumber\":\"6545435\"}},{\"createdBy\":\"155501\",\"createdDate\":\"2018-10-18T05:13:09.000+0000\",\"lastModifiedBy\":\"155501\",\"lastModifiedDate\":\"2018-10-18T05:13:09.000+0000\",\"modelId\":4,\"storageId\":\"155501\",\"siteId\":\"21424\",\"contentJson\":{\"Status\":\"Active\",\"DateOfBirth\":\"\",\"IDNumber2\":\"4564646346\",\"ContactNumberSingapore\":\"657547574574\",\"FirstName\":\"Biswo\",\"Gender\":\"Male\",\"Salutation\":\"Dr\",\"IDType\":\"{\\\"id\\\":\\\"FIN\\\",\\\"name\\\":\\\"FIN\\\"}\",\"PrimaryEmailAddress\":\"\",\"SecondaryEmailAddress\":\"\",\"IDType2\":\"{\\\"id\\\":\\\"NRIC\\\",\\\"name\\\":\\\"NRIC\\\"}\",\"UserId\":\"155501\",\"FullName\":\"ddfsfsf\",\"LastName\":\"Mohapatra\",\"ContactNumberOthers\":\"57457457457\",\"IDNumber\":\"674564\"}}],\"pageable\":{\"sort\":{\"unsorted\":true,\"sorted\":false,\"empty\":true},\"offset\":0,\"pageSize\":10,\"pageNumber\":0,\"paged\":true,\"unpaged\":false},\"totalPages\":1,\"last\":true,\"totalElements\":2,\"size\":10,\"number\":0,\"sort\":{\"unsorted\":true,\"sorted\":false,\"empty\":true},\"numberOfElements\":2,\"first\":true,\"empty\":false}";
	...
	public BooleanQuery buildBooleanQuery(long companyId,long groupId, SearchContext searchContext,
		...
		//TODO: SocialProfile doesnot have scopegroupid so below line had to be commented. So execute the line if search entry class is not socialprofile
		/*searchQuery.addRequiredTerm(Field.SCOPE_GROUP_ID,
				themeDisp.getScopeGroupId());*/

		// field search
		Map<String, BooleanQuery> conditionsMap = buildComponentConditionsMap(
				 searchContext, config,searchItems);
		...
		if (Validator.isNotNull(textSearchInput)) {
			try {
				...
				if (ArrayUtil.isEmpty(enabledComponentClasses) || (enabledComponentClasses.length == 1 && ArrayUtil.contains(enabledComponentClasses, StringPool.BLANK))) {
					return null;
				}
				Set<String> indexedFields = new HashSet<String>();
				for (String componentClass : enabledComponentClasses) {
					Indexer indexer = IndexerRegistryUtil
							.getIndexer(componentClass);
					Map<String, String> temp = indexer.getIndexedFields();
					indexedFields.addAll(temp.keySet());
				}
				indexedFields.removeAll(Arrays
						.asList(Field.UNSCORED_FIELD_NAMES));
				String queryString = StringPool.BLANK;
				if (config != null && config.getFreeTextSearch().equalsIgnoreCase(GenericSearchConstants.EXACT_MATCH)){
					textSearchInput = textSearchInput.trim().replaceAll(
							StringPool.SPACE + StringPool.PLUS, StringPool.SPACE);
					textSearchInput = textSearchInput.replaceAll(StringPool.SPACE,
							StringPool.STAR);
					textSearchInput = StringPool.QUOTE + textSearchInput
							+ StringPool.QUOTE;
					if (indexedFields.size() > 0) {
						for (String field : indexedFields) {
							queryString = queryString + StringPool.SPACE + field
									+ StringPool.COLON + textSearchInput
									+ StringPool.SPACE;
						}
		...
		BooleanQuery classesQuery = BooleanQueryFactoryUtil
				.create(searchContext);
	...
	public static BooleanQuery getBCForDate(String dateFilteringField, String startDate,String endDate, SearchContext searchContext){
        ...
        if(startCal != null && endCal != null){
        	 bq = getBC4RangeTerm(dateFilteringField, startCal, endCal, searchContext); // the dateFilteringField can be createDate or modified or displayDate
        }
	...
	private Map<String, BooleanQuery> buildComponentConditionsMap(
		...
		try {
			...
			//Map<String, String> componentsMap = getComponentsMap(req, themeDisp);
			...
			for (String strClass : componentClasses) {
				...
				if ("common".equals(strClass))
					continue;
				classesQuery.add(TermQueryFactoryUtil.create(searchContext,
						Field.ENTRY_CLASS_NAME, strClass),
						BooleanClauseOccur.MUST);
			}
			JSONArray jsonArray = selectedFs.getSearchItems();
			if(jsonArray != null){
				List<Integer> ignoreIndexes = new ArrayList<Integer>(
						jsonArray.length());
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					...
					String comp = obj
							.getString(GenericSearchConstants.KEY_COMPONENT);
					String compClass = obj
							.getString(GenericSearchConstants.KEY_COMPONENT_CLASS);
					String sectionType = obj
							.getString(GenericSearchConstants.KEY_SECTION_TYPE_KEY);
					// field name
					String field = obj
							.getString(GenericSearchConstants.KEY_TYPE_KEY);
					...
					for (int j = 0; (j < jsonArray.length()); j++) {
						...
						// vocabid or field or tag
						String otherObjType = otherObj
								.getString(GenericSearchConstants.KEY_TYPE);
						String otherObjComp = otherObj
								.getString(GenericSearchConstants.KEY_COMPONENT);
						String otherObjField = otherObj
								.getString(GenericSearchConstants.KEY_TYPE_KEY);
						otherObjsectionType = obj
								.getString(GenericSearchConstants.KEY_SECTION_TYPE_KEY);
						String otherObjopr = otherObj
								.getString(GenericSearchConstants.KEY_OPER);
						if (!otherObjComp.equals(comp)
								|| !otherObjType.equals(type)
								|| !otherObjField.equals(field))
							continue;
						if (Validator.isNull(otherObjopr))
							otherObjopr = "eq";
						if (!otherObjopr.equals(opr))
							continue;
						String otherObjValue = otherObj
								.getString(GenericSearchConstants.KEY_VALUE);
					...
					if((sectionType.equalsIgnoreCase(GenericSearchFilter.SELECT_AGERANGE)) || (otherObjsectionType.equalsIgnoreCase(GenericSearchFilter.SELECT_AGERANGE))){
						String[] splitValue = value.toString().replace(".00", "").split(",");
						// [0] - Indicates the min value chosen, [2] - Indicates the max value chosen
						// [2] -- Slider Min, [3] - Slider max
						// [4] - Indicates whether to consider slider values when their min and max are same as chosen min and max
						
						if ((GetterUtil.getInteger(splitValue[4]) == 1 && GetterUtil.getInteger(splitValue[0]) == GetterUtil.getInteger(splitValue[2]) && GetterUtil.getInteger(splitValue[1]) == GetterUtil.getInteger(splitValue[3]) 
								|| (GetterUtil.getInteger(splitValue[0]) != GetterUtil.getInteger(splitValue[2]) || GetterUtil.getInteger(splitValue[1]) != GetterUtil.getInteger(splitValue[3])))){
							if (isCombined) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("conditionMap after user filters = "
						+ conditionMap);
			}
			...
			if(config != null){
				...
				if (Validator.isNotNull(config.getFilters())) {
					for (GenericSearchFilter filter : filters) {
						if (filter.isDisplay())
							continue;
						BooleanQuery query = conditionMap
								.get(filter.getComponentClass());
						Map<String, Object> filterDataMap = GenericSearchHelper
								.getFilterDataMap(filter);
						String typeKey = (String) filterDataMap
								.get(GenericSearchConstants.KEY_TYPE_KEY);
						String sectionType = (String) filterDataMap
								.get(GenericSearchConstants.KEY_SECTION_TYPE_KEY);
						String opr = (String) filterDataMap
								.get(GenericSearchConstants.KEY_OPER);
						if (Validator.isNull(opr))
							opr = "eq";
						...
						if (GenericSearchFilter.SELECT_AGERANGE.equalsIgnoreCase(sectionType)){
							...
							if ((GetterUtil.getInteger(splitValue[4]) == 1 && GetterUtil.getInteger(splitValue[0]) == GetterUtil.getInteger(splitValue[2]) && GetterUtil.getInteger(splitValue[1]) == GetterUtil.getInteger(splitValue[3]) 
									|| (GetterUtil.getInteger(splitValue[0]) != GetterUtil.getInteger(splitValue[2]) || GetterUtil.getInteger(splitValue[1]) != GetterUtil.getInteger(splitValue[3])))){
								updateQueryWithConditions(query, searchContext, typeKey, sectionType,
										opr, filter.getValues());
							}
						} else{
							updateQueryWithConditions(query, searchContext, typeKey, sectionType,
									opr, filter.getValues());
			...
			if (logger.isDebugEnabled()) {
				logger.debug("conditionMap after background filters = "
						+ conditionMap);
			}
			return conditionMap;
		} catch (Exception e) {
			logger.error(
					"Error while building conditions maps for component, returning null",
					e);
			return null;
		}
	}

	private void updateQueryWithConditions(BooleanQuery query,
			SearchContext searchContext, String key, String sectionType, String opr, Object values)
			throws Exception {
		if (Validator.isNull(opr) || opr.equals("eq") || opr.equals("ne")) {
			BooleanQuery outerQuery = BooleanQueryFactoryUtil
					.create(searchContext);
			if (values instanceof List) {
				List<String> valuesList = (List<String>) values;
				for (String value : valuesList) {
					TermQuery innerQuery = TermQueryFactoryUtil.create(
							searchContext, key, value);
					outerQuery.add(innerQuery, BooleanClauseOccur.SHOULD);
				}
			} else {
				TermQuery innerQuery = TermQueryFactoryUtil.create(
						searchContext, key, values.toString());
				outerQuery.add(innerQuery, BooleanClauseOccur.SHOULD);
			}
			BooleanClauseOccur condition = opr.equals("ne") ? BooleanClauseOccur.MUST_NOT
					: BooleanClauseOccur.MUST;
			...
			BooleanQuery outerQuery = BooleanQueryFactoryUtil
					.create(searchContext);
			createRangeQuery(searchContext, key, sectionType, opr, null, start, end,
					outerQuery);
			query.add(outerQuery, BooleanClauseOccur.MUST);
		} else {
			...
			if (values instanceof List) {
				List<String> valuesList = (List<String>) values;
				for (String value : valuesList) {
					createRangeQuery(searchContext, key, sectionType, opr, value, start,
							end, outerQuery);
				}
			} else {
				createRangeQuery(searchContext, key, sectionType, opr, values.toString(),
						start, end, outerQuery);
			}
	...
	private void createRangeQuery(SearchContext searchContext, String key, String sectionType,
			String opr, String value, String start, String end,
			BooleanQuery outerQuery) throws ParseException {
		...
		TermRangeQuery rangeQuery = TermRangeQueryFactoryUtil.create(
				searchContext, key, start, end, includesLower, includesUpper);
	...
	private String checkIfTodayValue(String value) {
		...
		Format simpleDateFormat = FastDateFormatFactoryUtil
				.getSimpleDateFormat(DATE_FORMAT_PATTERN);
	...
	public XSSFWorkbook generateReport(PortletRequest req,
			ThemeDisplay themeDisplay, List<Document> docs) {
	...
	private void createRows(PortletRequest req, ThemeDisplay themeDisplay,
			List<Document> docs, XSSFWorkbook workbook,
			Map<String, String[]> columnMap) {
		PortletConfig portletConfig = (PortletConfig) req
				.getAttribute(JavaConstants.JAVAX_PORTLET_CONFIG);
		// format <vocabid_categoryid> = <category name>
		Map<String, String> categoryNames = new HashMap<String, String>();
		for (Document doc : docs) {
			try {
				String component = LanguageUtil.get(portletConfig,
						themeDisplay.getLocale(),
						doc.get(Field.ENTRY_CLASS_NAME));
				XSSFSheet sheet = workbook.getSheet(WordUtils
					...
					if (doc.hasField(col)) {
						...
						try {
							...
							if (Validator.isNotNull(date)) {
								value = DateUtil.getDate(date, "MMM dd, yyyy",
										themeDisplay.getLocale());
							} else {
						...
						handleAssetCategoryEntries(cell, doc, col,
								categoryNames);
					}
				}
			} catch (Exception e) {
	...
	private void handleAssetCategoryEntries(XSSFCell cell, Document doc,
			String col, Map<String, String> categoryNames) throws Exception {
		long categoryIds[] = GetterUtil.getLongValues(doc
		...
		for (Long categoryId : categories) {
			...
			if (Validator.isNull(categoryName)) {
				...
				categoryNames.put(assetCategory.getVocabularyId() + "_"
						+ categoryId, assetCategory.getName());
	...
	private void createSheetsAndHeaders(PortletRequest req,
			ThemeDisplay themeDisplay, XSSFWorkbook workbook,
			Map<String, String[]> columnMap) {
		...
		for (Entry<String, String> entry : componentsMap.entrySet()) {
			if ("common".equals(entry.getKey()))
				continue;
			Map<String, String> indexedFields = IndexerRegistryUtil.getIndexer(
					entry.getValue()).getIndexedFields();
			String sheetName = WordUtils.capitalize(entry.getKey());
			XSSFSheet sheet = workbook.createSheet(sheetName);

			String valuesString = req.getPreferences().getValue(
					GenericSearchConstants.PREF_EXPORTS + entry.getKey(), null);
			...
			cellStyle.setFillForegroundColor(new XSSFColor(new Color(200, 200,
					200)));
	...
	private void cleanup(XSSFWorkbook workbook) {
		for (int i = workbook.getNumberOfSheets() - 1; i >= 0; i--) {
			...
			short firstCellNum = sheet.getRow(sheet.getFirstRowNum())
					.getFirstCellNum();
	...
	// Class used to process json object containing filters selected during search
	...
	private static final Log logger = LogFactoryUtil
			.getLog(GenericSearchHelper.class);
