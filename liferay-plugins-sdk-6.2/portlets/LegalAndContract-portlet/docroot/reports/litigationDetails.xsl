...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="//recList">
			...
			<fo:table>
				<fo:table-body>
					<xsl:for-each select=".//entry">
						<xsl:if
							test="not(starts-with(key,'Response Deadline') or starts-with(key,'Remarks'))">
							<xsl:choose>
		...
		<fo:block font-size="12pt" linefeed-treatment="preserve">
			<xsl:for-each
				select=".//entry/key[starts-with(.,'Response Deadline') or starts-with(.,'Remarks') ]">
				<xsl:choose>
					<xsl:when test="position() mod 2 != 0">
						<xsl:variable name="val" select="../value"></xsl:variable>
						<fo:table>
							<fo:table-body>
								<fo:table-row>
									<fo:table-cell>
										<fo:block xsl:use-attribute-sets="extras">
											Response Deadline:
					...
					<xsl:otherwise>
						<fo:block xsl:use-attribute-sets="extras" margin-top="10pt" margin-left="40pt" linefeed-treatment="preserve">
							Remarks : 
							<xsl:value-of select="../value" />
						</fo:block>
					</xsl:otherwise>
