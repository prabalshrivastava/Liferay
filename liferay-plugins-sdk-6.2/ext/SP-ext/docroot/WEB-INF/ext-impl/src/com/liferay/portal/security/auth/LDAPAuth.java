...
public class LDAPAuth implements Authenticator {
	...
	protected int authenticate(
		...
		if(_log.isDebugEnabled()){
			_log.debug("Authenticating user using ldap - " + screenName);
		}
		...
		try {
			...
			if (enu.hasMoreElements()) {
				...
				if(_log.isDebugEnabled()){
					_log.debug("Authentication success.. will import user info from ldap");
				}
				...
				if(_log.isDebugEnabled()){
					_log.debug("User imported from ldap. Attributes " + attributes) ;
				}
				...
				// Naresh : 13th Jan 2015 Added below code  as Part of Menarini ESN
				try{
					createEsnLdapProfile(attributes, user);
				}catch(Exception ex){
					_log.error("Failed to create profile for user -> " + user.getScreenName() + ex.getMessage(),ex);
				}
	...
	// Naresh : 13th Jan 2015 Added below method as part of esn changes
	private void createEsnLdapProfile(Attributes attributes,User user) throws SystemException{
		String department = getAttributeValue(attributes.get(SambaashConstants.LDAP_DEPARTMENT));
		String company = getAttributeValue(attributes.get(SambaashConstants.LDAP_COMPANY));
		String country = getAttributeValue(attributes.get(SambaashConstants.LDAP_COUNTRY));
		if(Validator.isNull(department) || Validator.isNull(company) || Validator.isNull(country)){
			_log.error("User -> " + user.getScreenName() +"One of among department,company,country is null or empty,department= " + department + ", company = " + company + ", country=" + country);
		}else{
			SPLdapMapping esn = SPLdapMappingLocalServiceUtil.findByCountryDepartmentAndLegalEntity(country, department, company);
			if(Validator.isNull(esn)){
				_log.error("User -> " + user.getScreenName() +"Esn Mapping not found for department= " + department + ", company = " + company + ", country=" + country);
			}
			if(Validator.isNotNull(esn)){
				try{
					if(_log.isDebugEnabled()){
						_log.debug("Esn ldap mapping found for ldap dept = " +  department + " company=" + company + " country= " + country);
					}
					SPLdapProfile profile = SPLdapProfileLocalServiceUtil.findByUserId(user.getUserId());
					if(_log.isDebugEnabled()){
						_log.debug(" Fetching esn ldap profile for userId  " + user.getUserId());
					}
					if(Validator.isNull(profile)){
						if(_log.isDebugEnabled()){
							_log.debug("  esn ldap profile not exists for userId  " + user.getUserId() + " lets create it");
						}
						profile = SPLdapProfileLocalServiceUtil.getNewSPLdapProfile();
					}
...
//				profile.setGroupId(user.getGroupId());
					profile.setGroupId(esn.getGroupId());
					profile.setCountryId(esn.getCountryId());
					profile.setDepartmentId(esn.getDepartmentId());
					//		[sn, middleName, userPassword, sAMAccountName, givenName, userprincipalname, memberOf, cn]
					//			displayName,company,division,title,mail,employeeID,managertelephoneNumber,mobile,facsimileTelephoneNumber
					profile.setGivenName(getAttributeValue(attributes.get("givenName")));
					profile.setSn(getAttributeValue(attributes.get("sn")));
					profile.setDisplayName(getAttributeValue(attributes.get(SambaashConstants.LDAP_DISPLAYNAME)));
					profile.setCompany(company);
					profile.setDivision(getAttributeValue(attributes.get(SambaashConstants.LDAP_DIVISION)));
					profile.setTitle(getAttributeValue(attributes.get(SambaashConstants.LDAP_TITLE)));
					profile.setMail(getAttributeValue(attributes.get(SambaashConstants.LDAP_MAIL)));
					profile.setSamAccountName(getAttributeValue(attributes.get("sAMAccountName")));
					profile.setEmployeeId(getAttributeValue(attributes.get(SambaashConstants.LDAP_EMPLOYEE_ID)));
					profile.setManager(getAttributeValue(attributes.get(SambaashConstants.LDAP_MANAGER)));
					profile.setTelephoneNumber(getAttributeValue(attributes.get(SambaashConstants.LDAP_TELEPHONE_NUMBER)));
					profile.setMobile(getAttributeValue(attributes.get(SambaashConstants.LDAP_MOBILE)));
					profile.setFacsimileTelephoneNumber(getAttributeValue(attributes.get(SambaashConstants.LDAP_FACSIMILE_TELEPHONE_NUMBER)));
					
					
					if(_log.isDebugEnabled()){
						_log.debug("Updating esn ldap profile " + profile);
					}
					SPLdapProfileLocalServiceUtil.updateSPLdapProfile(profile);
					
				}catch(Exception ex){
					_log.error("Error while adding/updating esn ldap profile for user " + user.getUserId() + " Error " + ex);
				}
			}
		}
	}
	...
	protected int authenticate(
		...
		// Naresh: 14th Jan 2015 Added below code to check whether non-ad user or not. Done as part of menarini esn;
		...
		try {
			...
			_log.error("User " + screenName + " not found. will import from ldap");
		}
		
		boolean nonAdUser = SambaashUtil.isNonAdUser(user);
		if (nonAdUser) {
			if (_log.isDebugEnabled()) {
				_log.debug("Authenticator is not enabled");
			}
