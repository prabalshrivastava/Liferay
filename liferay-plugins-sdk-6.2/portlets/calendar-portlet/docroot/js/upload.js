...
DndUpload.prototype.handleDrop = function(e) {
	...
	var uploadFileResourceURLCatgId = this.uploadFileResourceURL + "&action=imageUpload";//	alert("uploadFileResourceURL " +this.uploadFileResourceURL + " #### selectedCategoryId ### " +selectedCategoryId);
	e.preventDefault();
	var elem;
	var files;
	var fileUpload;
...
function handleFiles(files, uploadFileResourceURL, removeFileResourceURL, saveLinkResourceUrl, portletNamespace, fileSizeMax, imageWidthMin, imageHeightMin, allowedTypes) {
	...
	var filesUploaded = document.getElementById('eventTempFileEntryId');
	commonFileMetadataContainer = document.getElementById(portletNamespace + 'commonFileMetadataContainer');
	for (var i = 0, len = files.length; i < len; i++) {
		fileUpload = new FileUpload(dropZone, commonFileMetadataContainer, files[i], uploadFileResourceURL + "&action=imageUpload", removeFileResourceURL, saveLinkResourceUrl, portletNamespace, fileSizeMax, imageWidthMin, imageHeightMin, allowedTypes);
	}
//}catch(err) {
	//alert(err);
//}
	...
	if (del) {
		if (this.xhr) {
			...
			if (isDuplicate) {
				...
				var eventTempFolderId = data.eventTempFolderId;
				var eventTempFileEntryId = data.fileEntryIds;
...
//				this.fileMetaDiv.appendChild(removeLink);
				var tempFileEntryIdElm = document.getElementById(this.portletNamespace + "eventTempFolderId");
				tempFileEntryIdElm.value = eventTempFolderId;
				var eventtempFileEntryIdElm = document.getElementById(this.portletNamespace + "eventTempFileEntryId");
				var existingVal = eventtempFileEntryIdElm.value;
				eventtempFileEntryIdElm.value = existingVal + "," + eventTempFileEntryId;
				console.log("eventTempFolderId : " + eventTempFolderId);
			}
		}
	}else {
		successFeedback.setAttribute('class', cls);
