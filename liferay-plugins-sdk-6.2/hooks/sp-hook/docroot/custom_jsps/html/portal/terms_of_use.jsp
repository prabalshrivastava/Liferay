...
<%@page import="com.sambaash.platform.srv.spservices.service.SPUserTypeLocalServiceUtil"%>
...
String articleId = (String)request.getAttribute("declarationId");
Long groupId = themeDisplay.getScopeGroupId();
String typeId =  String.valueOf(request.getAttribute("userTypeId"));
String siteId = String.valueOf(request.getAttribute("siteId"));
request.setAttribute("userTypeIdfwd" , typeId);
request.setAttribute("siteIdfwd" , siteId);
...
<aui:form action='<%= themeDisplay.getPathMain() + "/portal/update_terms_of_use" %>' name="fm">
	...
	<aui:input name="userTypeIdfwd" type="hidden" value="<%= typeId %>" /> 
    <aui:input name="siteIdfwd" type="hidden" value="<%= siteId %>" /> 
	<c:choose>
		...
		<c:when test="<%=Validator.isNumber(articleId) %>">
			<%@ include file="/html/portal/sp_declaration.jsp" %>
		</c:when>
		<c:otherwise>
