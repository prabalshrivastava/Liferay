...
public class CalendarUtil {
	...
	public static JSONObject getCalendarRenderingRules(ThemeDisplay themeDisplay, long[] calendarIds, int[] statuses,
			long startTime, long endTime, String ruleName, TimeZone timeZone) throws SystemException {

		List<CalendarBooking> calendarBookings = CalendarBookingLocalServiceUtil.search(themeDisplay.getCompanyId(),
				null, calendarIds, new long[0], -1, null, startTime, endTime, true, statuses, QueryUtil.ALL_POS,
		...
		for (CalendarBooking calendarBooking : calendarBookings) {
			...
			java.util.Calendar startTimeJCalendar = JCalendarUtil.getJCalendar(calendarBooking.getStartTime(),
					displayTimeZone);
			java.util.Calendar endTimeJCalendar = JCalendarUtil.getJCalendar(calendarBooking.getEndTime(),
					displayTimeZone);
	...
	public static Collection<CalendarResource> getCalendarResources(List<CalendarBooking> calendarBookings)
	...
	public static HashMap<String, List<String>> getEventsImageMap(long respositoryId, long calendarBookingId) {
		...
		try {
			...
			Folder parentFolder = DLAppServiceUtil
					.getFolder(respositoryId, 0, CalendarConstants.CALENDAR_PARENT_FOLDER);

			imageUrl = new ArrayList<String>();
			try {
				Folder subFolder = DLAppServiceUtil.getFolder(respositoryId, parentFolder.getFolderId(),
						CalendarConstants.CALENDAR_FOLDER + calendarBookingId);
				...
				eventImageFiles.put(String.valueOf(calendarBookingId), imageUrl);
				CalendarBooking calBook = CalendarBookingLocalServiceUtil.getCalendarBooking(calendarBookingId);
				User user = UserLocalServiceUtil.getUser(calBook.getUserId());
	...
	public static String[] getStatusStrings(Locale locale, int status) {

		// [0] - always holds localized string
		// [1] - always holds status in english - used to define css classes

		String[] strs = new String[2];

		if (status == CalendarBookingWorkflowConstants.STATUS_APPROVED) {
			strs[0] = LanguageUtil.get(locale, "accepted");
			strs[1] = "accepted";
		} else if (status == CalendarBookingWorkflowConstants.STATUS_PENDING) {
			strs[0] = LanguageUtil.get(locale, "pending");
			strs[1] = "pending";
		} else if (status == CalendarBookingWorkflowConstants.STATUS_MAYBE) {
			strs[0] = LanguageUtil.get(locale, "maybe");
			strs[1] = "maybe";
		} else if (status == CalendarBookingWorkflowConstants.STATUS_DENIED) {
			strs[0] = LanguageUtil.get(locale, "declined");
			strs[1] = "declined";
		}
	...
	public static JSONObject toCalendarBookingJSONObject(ThemeDisplay themeDisplay, CalendarBooking calendarBooking,
		...
		jsonObject.put("eventCreator", calendarBooking.getUserName());
		jsonObject.put("title", calendarBooking.getTitle(themeDisplay.getLocale()));

		return jsonObject;
	}

	public static JSONArray toCalendarBookingsJSONArray(ThemeDisplay themeDisplay,
			List<CalendarBooking> calendarBookings) throws PortalException, SystemException {
	...
	public static JSONArray toCalendarBookingsJSONArray(ThemeDisplay themeDisplay,
			List<CalendarBooking> calendarBookings, TimeZone timeZone) throws PortalException, SystemException {
	...
	public static JSONObject toCalendarJSONObject(ThemeDisplay themeDisplay, Calendar calendar) throws SystemException {
		...
		CalendarResource calendarResource = CalendarResourceLocalServiceUtil.fetchCalendarResource(calendar
	...
	public static JSONArray toCalendarsJSONArray(ThemeDisplay themeDisplay, List<Calendar> calendars)
	...
	private static JSONObject _getPermissionsJSONObject(PermissionChecker permissionChecker, Calendar calendar) {
		...
		jsonObject.put(ActionKeys.MANAGE_BOOKINGS,
				CalendarPermission.contains(permissionChecker, calendar, ActionKeys.MANAGE_BOOKINGS));

		jsonObject.put(ActionKeys.PERMISSIONS,
				CalendarPermission.contains(permissionChecker, calendar, ActionKeys.PERMISSIONS));
		...
		jsonObject.put(ActionKeys.VIEW_BOOKING_DETAILS,
				CalendarPermission.contains(permissionChecker, calendar, ActionKeys.VIEW_BOOKING_DETAILS));

		return jsonObject;
	}

	private static Log _log = LogFactoryUtil.getLog(CalendarUtil.class);

	private static TimeZone _utcTimeZone = TimeZone.getTimeZone(StringPool.UTC);

	public static HashMap<String, List<String>> getEventsImageMap(
			long respositoryId, long calendarBookingId, User user) {
		HashMap<String, List<String>> eventImageFiles = new HashMap<String, List<String>>();
		List<String> imageUrl = null;
