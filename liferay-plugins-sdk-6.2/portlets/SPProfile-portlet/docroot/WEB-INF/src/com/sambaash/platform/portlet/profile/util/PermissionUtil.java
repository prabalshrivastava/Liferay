package com.sambaash.platform.portlet.profile.util;
...
public class PermissionUtil {
	...
	public static String getPermittedNavConfig(ThemeDisplay themeDisplay, PortletPreferences portletPreferences,
	        String userId) {
		String navConfigStr = portletPreferences.getValue(ProfileConstants.NAV_CONFIG, "{}");

		if (SambaashUtil.isAdmin(themeDisplay.getScopeGroupId(), themeDisplay.getUserId())) {
			return navConfigStr;
		}
		if (Validator.isNotNull(userId) && !userId.isEmpty()) {
			if (themeDisplay.getUserId() == Long.parseLong(userId)) {
				// return navConfigStr;
			}
		}
		...
		try {
			...
			if (configObj.has("menus") && configObj.getJSONArray("menus").length() > 0) {
				JSONArray updatedMenuArr = updateMenus(getUserRoleNames(Long.valueOf(userId)), configObj);
		...
		logger.error("Final nav config = " + updatedConfig);
	...
	private static JSONArray updateMenus(List<String> userRoles, JSONObject configObj) {
		...
		for (int i = 0; i < menuArr.length(); i++) {
			...
			updatedMenuObj.put("statusLabel", menuObj.getString("statusLabel"));
			updatedMenuObj.put("updateUrl", menuObj.getString("updateUrl"));
			...
			for (int j = 0; j < menuItemsArr.length(); j++) {
				...
				if (subMenuItemsArr.length() > 0) {
					...
					for (int k = 0; k < subMenuItemsArr.length(); k++) {
						...
						updateViewableMenuItems(userRoles, subMenuItemObj, updatedSubMenuItemsArr);
					}
					...
					updateViewableMenuItems(userRoles, menuItemObj, updatedMenuItemsArr);
				}
			}
	...
	private static void updateViewableMenuItems(List<String> userRoles, JSONObject menuItemObj,
	        JSONArray updatedMenuItemsArr) {
		if (menuItemObj.has("viewableBy")) {
			JSONArray viewableByArr = menuItemObj.getJSONArray("viewableBy");
			boolean viewOk = false;
			if (viewableByArr.length() > 0) {
