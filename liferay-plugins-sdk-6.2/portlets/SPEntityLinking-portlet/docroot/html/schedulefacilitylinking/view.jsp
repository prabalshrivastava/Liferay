...
<div class="subHeader">
	<div class="container">
		...
		<aui:row>
			<aui:col span="10" cssClass="text-center"><h2><span>Schedule Linking</span></h2></aui:col>
				...
				<div class="entitySearch">
					<aui:row>
						<aui:col span="12">
							...
							<aui:input type="text"  name="left" placeholder="Search for a ${leftModel} title or code" onchange="getList('Schedule','left');"></aui:input>
						</aui:col>
					</aui:row>
					<aui:row>
				...
				<div class="entitySearch">
					<aui:row>
						...
						<div class="selectedItem slectedItem">
							...
							<aui:input type="text"  name="right" placeholder="Search for a ${rightModel} title or code" onchange="getList('Facility','right');"></aui:input>
						</aui:col>
					</aui:row>
					<aui:row>
...
var fetchLeftLinkType = "ScheduleFacilityEntityLink";
...
function getList(modelName,side){
	...
	var sortby = [];
	if(side == "left"){
		data["sortby"] = [{"direction":"asc","field":"Name","contentJSON":"true"}];	
		data["searchIn"]= ['contentJson.Name','contentJson.ScheduleCode'];
		
		filter["contentJson.Category"] = ["Subject","Briefing"];	
		filter["contentJson.ScheduleStatus"] = ["Confirmed"];	
	}else{
		data["sortby"] = [{"direction":"asc","field":"FacilityName","contentJSON":"true"}];	
		data["searchIn"]= ['contentJson.FacilityCode','contentJson.FacilityName'];
	}
...
function reloadListing(div,data){
	...
	for(var i =0; i <data.length; i++){
		var obj = {};
		if(div == "programmeDiv"){
			var code = data[i].contentJson.ScheduleCode
			var name = data[i].contentJson.Name
			obj.key = code ;
			obj.value = name;
			obj.endDate = data[i].contentJson.EndDate;
			...
			if(data[i].contentJson.hasOwnProperty('SubFacilityList')){
				var SFList=data[i].contentJson.SubFacilityList;
				for(var s =0; s <SFList.length; s++){
					var code = data[i].contentJson.FacilityCode
					var name = data[i].contentJson.FacilityName
					obj.key = code +((SFList[s].SFCode.length >0) ? ' -- '+ code+SFList[s].SFCode : '');
					obj.value = name +((SFList[s].SFName.length >0) ? ' - '+SFList[s].SFName : '');
				...
				obj.key = data[i].contentJson.FacilityCode
				obj.value = data[i].contentJson.FacilityName + rightSideList.push(obj);
				var clone = listElement.cloneNode(true);
				clone.style.display = "block";
...
function saveLink(){
	...
	var data = {
			"linkType":"ScheduleFacilityEntityLink",
			"formType":"EntityLink",
			"ModelRight":modelRight,
			"ModelLeft":modelLeft,
			"ModelLeft1":"",
			"ModelRight1":"SubFacility",
			"records":records
