		...
		<li id="level2">
			...
			<div class="content toggler-content-expanded" style="display:block;">
				...
				<#list nav_items as nav_item>
                    ...
                    <#assign pageUrl =  nav_item.getURL()>
                    <#if nav_item.getURL()?contains("corporate-profile-edit")>
                        <#if usrStartupList.size() != 0>
                            <#list usrStartupList as prof>
                                <#assign pageUrl = prof.getEditUrl()>
                            </#list>
                        </#if>
                    </#if>

                    <#if !nav_item.hasChildren()>
                        <#if  usrStartupList.size() <1 && nav_item.getURL()?contains("corporate-profile-edit")>
                        <#else>
                            <h4 class="level-2-header" id="h4-header-${counter1}">
                            <a aria-labelledby="layout_${nav_item.getLayoutId()}" ${nav_item_attr_has_popup} href="${pageUrl}" ${nav_item.getTarget()} role="menuitem"><span>${nav_item.icon()} ${nav_item.getName()}</span></a>
                            </h4>
                        </#if>
                    </#if>
										...
										<#list nav_child.getChildren() as nav_child_item>
											...
											<li ${nav_child_item_attr_selected} class="${nav_child_item_css_class}" id="layout_${nav_child_item.getLayoutId()}" role="presentation">
												...
												<#if !nav_child_item.hasChildren()>
													<h4 class="level-2-header" id="h4-header-">
												...
												<#if nav_child_item.hasChildren()>
													<h4 class="level-2-header header toggler-header-collapsed" id="h4-header-">
														<a aria-labelledby="layout_${nav_child_item.getLayoutId()}" ${nav_child_item_attr_has_popup} href="${nav_child_item.getURL()}" ${nav_child_item.getTarget()} role="menuitem"><span>${nav_child_item.icon()} ${nav_child_item.getName()}</span></a>
													</h4>
												</#if>
