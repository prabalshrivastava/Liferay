...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="//recList">
			...
			<fo:table>
				<fo:table-body>
					<xsl:for-each select=".//entry">
						<xsl:if
							test="not(starts-with(key,'Class Code') or starts-with(key,'Class Speci') or starts-with(key,'History') or starts-with(key,'Remarks'))">
							<xsl:choose>
		...
		<fo:block xsl:use-attribute-sets="gap">
			<xsl:for-each
				select=".//entry/key[starts-with(.,'Class Code') or starts-with(.,'Class Speci')]">
				<xsl:choose>
					<xsl:when test="position() mod 2 != 0">
		...
		<fo:block xsl:use-attribute-sets="gap">
			<xsl:for-each
				select=".//entry/key[starts-with(.,'History')]">
				<fo:block margin-top="40pt" xsl:use-attribute-sets="fontFamily">
					History
				</fo:block>
				<fo:block margin-left="20pt" margin-top="20pt" linefeed-treatment="preserve" xsl:use-attribute-sets="fontFamily">
