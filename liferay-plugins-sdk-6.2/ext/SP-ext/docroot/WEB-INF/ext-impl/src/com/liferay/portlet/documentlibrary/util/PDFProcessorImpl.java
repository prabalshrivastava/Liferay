...
public class PDFProcessorImpl
	...
	protected boolean hasPreview(FileVersion fileVersion, String type)
		...
		_log.debug("PdfProcessoriMpl previewFilePath :" + previewFilePath);
		
		_log.debug("PdfProcessoriMpl has file " +DLStoreUtil.hasFile(
	...
	//	if (_isGenerateThumbnail(fileVersion)) {
		// Naresh : April 19,2016 - using hasThumbnails method instead _isGenerateThumbnail
		// _isGenerateThumbnail checks only default thumbnail, whereas hasThumbnails checks custom sized thumbnails as well..
		// _hasImages method uses hasThumbnails, here also same check must be done.
