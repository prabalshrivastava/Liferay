		...
		<div class="journal-article-container" id="<portlet:namespace />journalArticleContainer">
			<c:choose>
				...
				<c:otherwise>
					...
					<div>
						<div class="collapse_buttons" style="text-align:right">
							<div class="expand_all" style="display:inline-block"><input type="button" onclick="javascript:expandAll();" value="Expand All"/></div>
							<div class="collapse_all" style="display:inline-block"><input type="button" onclick="javascript:collapseAll();" value="Collapse All"/></div>
						</div>
...
function expandAll() {
var A = AUI().use('aui-base', 'aui-node');
	
	var toggleIconNodes = A.all('.toggler_icon');
	
	for (var i = 0; i < toggleIconNodes.size(); i++){
		var fieldNode = toggleIconNodes._nodes[i].parentElement;
		
		var children = fieldNode.childNodes;
		for (var j = 0; j < children.length; j++) {
			if (children[j].className || children[j].nodeName == "DIV"){
				if ((!children[j].classList.contains('toggler_icon')) && (!children[j].classList.contains('control-label')) && (!children[j].classList.contains('repeataable_icons'))) {
	...
	/*var items = A.all('.lfr-ddm-container');
	items.each(function(item) {
		try {
			//setCalculatedHeight(item);
			//item.addClass("active");
			item.removeClass("hide");
			//var toggleIcon = item.siblings('.toggler_icon').item(0);
			//toggleIcon.removeClass('toggler_icon_collapse');
			//toggleIcon.addClass('toggler_icon_expand');
		} catch (err) {
			console.log(err);
		}
	});*/
...
function collapseAll() {
	...
	items.each(function(item) {
		try {
			...
			//var toggleIcon = item.siblings('.toggler_icon').item(0);
			//toggleIcon.removeClass('toggler_icon_expand');
			//toggleIcon.addClass('toggler_icon_collapse');
		} catch (err) {
			console.log(err);
		}
	});*/
