...
<div id="<portlet:namespace />inviteFriendsEmailContent">
	<form id="<portlet:namespace />ip_form" action="<%=inviteFriendsURL %>" method="POST" onsubmit="sendInvitations(event)" class="s1">
		...
		<div class="fieldset">
			<div><label for="ip_to" class="sp-group-column sp-group-of-ten" style="vertical-align: top;"><span class="sp-group-ui-db">Share to:</span></label><div class="sp-group-nonuple-column sp-group-column sp-group-of-ten"><div data-autocomplete-dom-id="sis-holder" data-group-id="<%=folderId %>" class="ip-to">
				<ul data-autocomplete-dom-id="selected-items" class="horizontal sp-group-ui-dib sp-group-ui-vam">
					
				</ul>
				<span class="ip-to-input-outer"><span style="visibility: hidden; z-index: -10000; position: relative; width: 100%;">Add Email Address&nbsp;</span><input data-autocomplete-dom-id="input" id="ip_to" type="text" tabindex="0" placeholder="Add Email Address" autocomplete="off" value="" class="ip-to-input" /></span></div>
		...
		<div>
			<span class="sp-group-column sp-group-of-ten"></span><input id="ag_submit" type="submit" value="Internal Share" tabindex="3" class="sp-group-column" />
		</div>
	</form>
</div>
	...
	*/
	...
	try{
        ...
        if(send_to.length == 0) {
			...
			temp_url += "&folderId=" + folderId + "&fileEntryId=" + file_entry_id + "&send_to=" + send_to +"&invite_by=" + invite_by+"&shareType=1&durationList=" + durationList + "&permissionList=" + rwList;
