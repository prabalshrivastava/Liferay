...
public class EditGroupAction extends PortletAction {
		...
		try {
			if (cmd.equals(Constants.ADD) || cmd.equals(Constants.UPDATE)) {
				...
				if (cmd.equals(Constants.ADD) && layoutGroup.isControlPanel()) {
					...
					PortletURL siteAdministrationURL = PortalUtil
							.getSiteAdministrationURL(actionResponse,
									themeDisplay, PortletKeys.SITE_SETTINGS);

					String controlPanelURL = HttpUtil.setParameter(
							themeDisplay.getURLControlPanel(), "p_p_id",
							PortletKeys.SITES_ADMIN);

					controlPanelURL = HttpUtil.setParameter(controlPanelURL,
							"controlPanelCategory",
							themeDisplay.getControlPanelCategory());

					siteAdministrationURL.setParameter("redirect",
							controlPanelURL);
					...
					redirect = HttpUtil.setParameter(redirect, "doAsGroupId",
							group.getGroupId());
					redirect = HttpUtil.setParameter(redirect, "refererPlid",
							newRefererPlid);

					closeRedirect = updateCloseRedirect(closeRedirect, group,
							themeDisplay, oldFriendlyURL, oldStagingFriendlyURL);
				}
			...
			if (e instanceof NoSuchGroupException
				...
				int stagingType = ParamUtil.getInteger(actionRequest,
						"stagingType");

				if (stagingType != StagingConstants.TYPE_NOT_STAGED) {
					redirect = HttpUtil.setParameter(redirect,
							actionResponse.getNamespace() + "stagingType",
							stagingType);
				}
	 ...
	 *
	 * @param actionRequest
	 *            the portlet request used to retrieve parameters
	 * @throws Exception
	...
	protected void resetMergeFailCountAndMerge(ActionRequest actionRequest)
		...
		long groupId = ParamUtil.getLong(actionRequest, "groupId");
		boolean privateLayoutSet = ParamUtil.getBoolean(actionRequest,
				"privateLayoutSet");
		...
		layoutSetPrototype = LayoutSetPrototypeServiceUtil
				.getLayoutSetPrototype(layoutSetPrototypeId);
	...
	protected void updateActive(ActionRequest actionRequest, String cmd)
		...
		GroupServiceUtil.updateGroup(groupId, group.getParentGroupId(),
				group.getName(), group.getDescription(), group.getType(),
				group.isManualMembership(), group.getMembershipRestriction(),
				group.getFriendlyURL(), active, serviceContext);
	}

	protected String updateCloseRedirect(String closeRedirect, Group group,
			ThemeDisplay themeDisplay, String oldFriendlyURL,
			String oldStagingFriendlyURL) throws PortalException,
			SystemException {
		...
		if (Validator.isNotNull(oldFriendlyURL)) {
			...
			if (closeRedirect.contains(oldPath)) {
				closeRedirect = PortalUtil.updateRedirect(closeRedirect,
						oldPath, newPath);
			} else {
				closeRedirect = PortalUtil.getGroupFriendlyURL(group, false,
						themeDisplay);
			}
		}
	...
	protected Object[] updateGroup(ActionRequest actionRequest)
		...
		long parentGroupId = ParamUtil.getLong(actionRequest,
		...
		if (liveGroupId <= 0) {
			...
			manualMembership = ParamUtil.getBoolean(actionRequest,
					"manualMembership");

			liveGroup = GroupServiceUtil.addGroup(parentGroupId,
					GroupConstants.DEFAULT_LIVE_GROUP_ID, name, description,
					type, manualMembership, membershipRestriction, friendlyURL,
					true, active, serviceContext);

			LiveUsers.joinGroup(themeDisplay.getCompanyId(),
					liveGroup.getGroupId(), userId);
		} else {
			...
			type = ParamUtil.getInteger(actionRequest, "type",
					liveGroup.getType());
			friendlyURL = ParamUtil.getString(actionRequest, "friendlyURL",
					liveGroup.getFriendlyURL());
			active = ParamUtil.getBoolean(actionRequest, "active",
					liveGroup.getActive());
			manualMembership = ParamUtil.getBoolean(actionRequest,
					"manualMembership", liveGroup.isManualMembership());

			liveGroup = GroupServiceUtil.updateGroup(liveGroupId,
					parentGroupId, name, description, type, manualMembership,
					membershipRestriction, friendlyURL, active, serviceContext);

			if (type == GroupConstants.TYPE_SITE_OPEN) {
				List<MembershipRequest> membershipRequests = MembershipRequestLocalServiceUtil
						.search(liveGroupId,
								MembershipRequestConstants.STATUS_PENDING,
								QueryUtil.ALL_POS, QueryUtil.ALL_POS);

				for (MembershipRequest membershipRequest : membershipRequests) {
					MembershipRequestServiceUtil.updateStatus(membershipRequest
							.getMembershipRequestId(), themeDisplay
							.translate("your-membership-has-been-approved"),
			...
			try {
				// Add Sambaash custom logic
				Map<String, String[]> parameterMap = actionRequest
						.getParameterMap();
				JSONArray arr = JSONFactoryUtil.createJSONArray();
				Set<String> errorMessages = new HashSet<String>();
				for (Entry<String, String[]> entry : parameterMap.entrySet()) {
					String key = entry.getKey();
					if (!key.startsWith(SambaashConstants.SITE_SETTINGS.ATTR_PG_REDIRECT_ACTION_PAGE))
						continue;
					String index = key.substring(key.lastIndexOf("-") + 1);
					JSONObject obj = JSONFactoryUtil.createJSONObject();
					String url = entry.getValue()[0];
					if (Validator.isNull(url)) {
						errorMessages.add("error.key1");
						continue;
					}
					String forgetAfterProcess = parameterMap
							.get(SambaashConstants.SITE_SETTINGS.ATTR_PG_REDIRECT_FORGET
									+ "-" + index)[0];
					String requireLogin = parameterMap
							.get(SambaashConstants.SITE_SETTINGS.ATTR_PG_LOGIN_REQUIRED
									+ "-" + index)[0];
					obj.put(SambaashConstants.SITE_SETTINGS.ATTR_PG_REDIRECT_ACTION_PAGE,
							url);
					obj.put(SambaashConstants.SITE_SETTINGS.ATTR_PG_REDIRECT_FORGET,
							forgetAfterProcess);
					obj.put(SambaashConstants.SITE_SETTINGS.ATTR_PG_LOGIN_REQUIRED,
							requireLogin);
					arr.put(obj);
				}
				JSONArray arr1 = JSONFactoryUtil.createJSONArray();
				for (Entry<String, String[]> entry : parameterMap.entrySet()) {
					String key = entry.getKey();
					if (!key.startsWith(SambaashConstants.SITE_SETTINGS.ATTR_ACT_REDIRECT_ACTION_PAGE))
						continue;
					String index = key.substring(key.lastIndexOf("-") + 1);
					JSONObject obj = JSONFactoryUtil.createJSONObject();
					String url = entry.getValue()[0];
					if (Validator.isNull(url)) {
						errorMessages.add("error.key2");
						continue;
					} 
					String params = parameterMap
							.get(SambaashConstants.SITE_SETTINGS.ATTR_ACT_REDIRECT_ACTION_PARAMS
									+ "-" + index)[0];
					String forgetAfterProcess = parameterMap
							.get(SambaashConstants.SITE_SETTINGS.ATTR_ACT_REDIRECT_FORGET
									+ "-" + index)[0];
					String requireLogin = parameterMap
							.get(SambaashConstants.SITE_SETTINGS.ATTR_ACT_LOGIN_REQUIRED
									+ "-" + index)[0];
					obj.put(SambaashConstants.SITE_SETTINGS.ATTR_ACT_REDIRECT_ACTION_PAGE,
							url);
					obj.put(SambaashConstants.SITE_SETTINGS.ATTR_ACT_REDIRECT_ACTION_PARAMS,
							params);
					if (Validator.isNull(params)) {
						errorMessages.add("error.key3");
						continue;
					}
					obj.put(SambaashConstants.SITE_SETTINGS.ATTR_ACT_REDIRECT_FORGET,
							forgetAfterProcess);
					obj.put(SambaashConstants.SITE_SETTINGS.ATTR_ACT_LOGIN_REQUIRED,
							requireLogin);
					arr1.put(obj);
				}
		...
		String customJspServletContextName = ParamUtil.getString(actionRequest,
				"customJspServletContextName", typeSettingsProperties
						.getProperty("customJspServletContextName"));

		typeSettingsProperties.setProperty("customJspServletContextName",
				customJspServletContextName);

		typeSettingsProperties.setProperty("defaultSiteRoleIds", ListUtil
				.toString(getRoles(actionRequest), Role.ROLE_ID_ACCESSOR,
						StringPool.COMMA));
		typeSettingsProperties.setProperty("defaultTeamIds", ListUtil.toString(
		...
		for (String analyticsType : analyticsTypes) {
			if (StringUtil.equalsIgnoreCase(analyticsType, "google")) {
				String googleAnalyticsId = ParamUtil
						.getString(actionRequest, "googleAnalyticsId",
				...
				String analyticsScript = ParamUtil.getString(actionRequest,
						Sites.ANALYTICS_PREFIX + analyticsType,
						typeSettingsProperties.getProperty(analyticsType));

				typeSettingsProperties.setProperty(Sites.ANALYTICS_PREFIX
						+ analyticsType, analyticsScript);
			}
		}

		String publicRobots = ParamUtil.getString(actionRequest,
				"publicRobots",
				liveGroup.getTypeSettingsProperty("false-robots.txt"));
		String privateRobots = ParamUtil.getString(actionRequest,
				"privateRobots",
		...
		boolean trashEnabled = ParamUtil.getBoolean(actionRequest,
				"trashEnabled", GetterUtil.getBoolean(
						typeSettingsProperties.getProperty("trashEnabled"),
						true));

		typeSettingsProperties.setProperty("trashEnabled",
		...
		double trashEntriesMaxAgeGroup = ParamUtil.getDouble(actionRequest,
				"trashEntriesMaxAge");
		...
		if (trashEntriesMaxAgeGroup != trashEntriesMaxAgeCompany) {
			typeSettingsProperties.setProperty("trashEntriesMaxAge", String
					.valueOf(GetterUtil.getInteger(trashEntriesMaxAgeGroup)));
		} else {
		...
		int contentSharingWithChildrenEnabled = ParamUtil.getInteger(
				...
				GetterUtil.getInteger(typeSettingsProperties
						.getProperty("contentSharingWithChildrenEnabled"),
		...
		String publicVirtualHost = ParamUtil.getString(actionRequest,
				"publicVirtualHost", publicLayoutSet.getVirtualHostname());

		LayoutSetServiceUtil.updateVirtualHost(liveGroup.getGroupId(), false,
				publicVirtualHost);

		LayoutSet privateLayoutSet = liveGroup.getPrivateLayoutSet();

		String privateVirtualHost = ParamUtil.getString(actionRequest,
				"privateVirtualHost", privateLayoutSet.getVirtualHostname());

		LayoutSetServiceUtil.updateVirtualHost(liveGroup.getGroupId(), true,
				privateVirtualHost);
		...
		if (liveGroup.hasStagingGroup()) {
			...
			friendlyURL = ParamUtil.getString(actionRequest,
					"stagingFriendlyURL", stagingGroup.getFriendlyURL());

			GroupServiceUtil.updateFriendlyURL(stagingGroup.getGroupId(),
					friendlyURL);

			LayoutSet stagingPublicLayoutSet = stagingGroup
					.getPublicLayoutSet();

			publicVirtualHost = ParamUtil.getString(actionRequest,
					"stagingPublicVirtualHost",
					stagingPublicLayoutSet.getVirtualHostname());

			LayoutSetServiceUtil.updateVirtualHost(stagingGroup.getGroupId(),
					false, publicVirtualHost);

			LayoutSet stagingPrivateLayoutSet = stagingGroup
					.getPrivateLayoutSet();

			privateVirtualHost = ParamUtil.getString(actionRequest,
					"stagingPrivateVirtualHost",
					stagingPrivateLayoutSet.getVirtualHostname());

			LayoutSetServiceUtil.updateVirtualHost(stagingGroup.getGroupId(),
					true, privateVirtualHost);
		}

		liveGroup = GroupServiceUtil.updateGroup(liveGroup.getGroupId(),
		...
		if (!liveGroup.isStaged()) {
			...
			if ((privateLayoutSetPrototypeId == 0)
				...
				long layoutSetPrototypeId = ParamUtil.getLong(actionRequest,
						"layoutSetPrototypeId");
				int layoutSetVisibility = ParamUtil.getInteger(actionRequest,
						"layoutSetVisibility");
			...
			SitesUtil.updateLayoutSetPrototypesLinks(liveGroup,
		...
		long refererPlid = GetterUtil.getLong(HttpUtil.getParameter(redirect,
				"refererPlid", false));
		...
		if (!privateLayoutSet.isLayoutSetPrototypeLinkActive()
			...
			if ((refererPlid > 0)
					...
					&& (themeDisplay.getScopeGroupId() != liveGroup
							.getGroupId())) {
