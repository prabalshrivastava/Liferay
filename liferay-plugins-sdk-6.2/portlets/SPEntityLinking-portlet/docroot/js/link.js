function getList(modelName, callback) {
	var jsonfilterparameter = {"limit":1000,"modelName":modelName,"page":0,"formType":modelName};
	var filterData = [];
	...
	if(modelName == "Subject-Schedule"){
		jsonfilterparameter['modelName'] = "Schedule";
		jsonfilterparameter['formType'] = "Schedule";
		jsonfilterparameter["sortby"] = [{"direction":"asc","field":"Name","contentJSON":"true"}];
		jobValue = document.getElementById(namespace + "right").value ;
		filter["contentJson.Category"] = ["Subject"];	
		filterData.push(filter);
	}
...
function populatePriceSchemes(data) {
	reloadListing("subjectDiv", data, "ScheduleCode", "Name", "right");
}
...
function leftDropdownChanged() {
	...
	checkNone(document.getElementById("subjectDiv"), "right");	
	...
	if(left == "programme") {
		...
		getList("EntityLink", function(data) {
			...
			fetchLeftLinkType = "ProgrammeScheduleSubjectScheduleEntityLink";
			populateProgAndSchedules("programmeDiv", "left");
		});
	}
	showNextBtn();
...
function rightDropdownChanged() {
	removeElements("subjectDiv", "listElement");
	var right = document.getElementById("rightDropdown").value;
	if(right == "price-scheme") {
		
		getList("Subject-Schedule", function(data){
