function AutoComplete(param) {
		...
		if(mailIds){
			...
			for (var i = 0; i < emailArray.length; i++) {
				try{
					...
					var tempIndex = ne.indexOf("(");
					if(tempIndex != -1){
						name = ne.substring(0, tempIndex );
						email = ne.substring(tempIndex+1, ne.length-1);
					}else{
						email = ne;
					}
					
					createSelectedItem(email,null,null,name);
				}catch( er){
	...
	try{	
		...
		var si_html = "<li data-autocomplete-dom-id=\"si\" class=\"ip-si sp-group-maxs\"><div class=\"sp-group-clearfix\"><div class=\"sp-group-lfloat sp-group-mrm\" style=\"line-height: 0;\"><img data-autocomplete-dom-id=\"si-img\" src=\"\" class=\"sp-group-thumb_32\" alt=\"Auto Complete Image\"/></div><div class=\"sp-group-ui-dib\"><div class=\"sp-group-ui-dib sp-group-ui-vam\" style=\"height:32px;\"></div><div class=\"sp-group-ui-dib sp-group-ui-vam\"><span data-autocomplete-dom-id=\"si-name\"></span>(<span userIdCheck=\"userId\" data-autocomplete-dom-id=\"si-email\"></span>)</div></div>"
			+ "<div class=\"sp-group-ui-dib\"><div><span data-autocomplete-dom-id=\"si-x\">X</span></div></div></div></li>";
//			+ "<div style=\"float:right\"></div> </div></li>";
		...
		if(email_ul.length > 0){
			for(var i=0; i<email_ul.length;i++){
				...
				if(existingEmail.indexOf(email) >-1){
					emailExists = true;
					alert("Selected User already added. Please select a different user.");
					that.j_autocomplete_input.value = "";
					break;
				}
			}
		...
		if(emailExists == false){
			user_img = user_img ? user_img: "/../image/user_male_portrait?img_id=0";
	...
	try{	
		var si_html = "<li emailCheck=\"email\" data-autocomplete-dom-id=\"si\" class=\"ip-si sp-group-maxs\"><div class=\"sp-group-clearfix\"><div class=\"sp-group-lfloat sp-group-mrm\" style=\"line-height: 0;\"><img data-autocomplete-dom-id=\"si-img\" src=\"/../image/user_male_portrait?img_id=0\" class=\"sp-group-thumb_32\" alt=\"Auto Complete Image\"/></div><div class=\"sp-group-ui-dib\"><div class=\"sp-group-ui-dib sp-group-ui-vam\" style=\"height:32px;\"></div><div class=\"sp-group-ui-dib sp-group-ui-vam\"><span data-autocomplete-dom-id=\"si-email\"></span></div></div> " 
			+ "<div class=\"sp-group-ui-dib\"><div><span data-autocomplete-dom-id=\"si-x\" >X</span></div></div></div></li>"; 
...
//    		var temp_url = find_suggestions_url + "&group_id=" + group_id + "&q=" + input_value + "&invite_by=" + invite_by +"&sharedTypeToGetUserList=internalShare";
	...
	this.removeAllEmails = function(){
		var myNode = that.j_autocomplete_selected_items;
