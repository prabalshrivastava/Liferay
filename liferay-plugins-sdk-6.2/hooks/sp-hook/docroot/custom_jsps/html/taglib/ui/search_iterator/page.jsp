...
String parameter = GetterUtil.getString(SambaashUtil.getParameter("control.panel.link.profile", themeDisplay.getScopeGroupId()));
...
<div class="lfr-search-container <%= resultRows.isEmpty() ? "hide" : StringPool.BLANK %>">
	...
	<div id="<%= namespace + id %>SearchContainer">
				...
				%>
					...
					>
						...
						<c:if test="<%= orderKey != null %>">
							<div class="table-sort-liner thisisthejsp">
		...
		for (int i = 0; i < resultRows.size(); i++) {
			...
			String customProfileUrl = StringPool.BLANK;
			User user2 = null;
			try{
				user2 = (User)row.getObject();
				customProfileUrl = "/"+user2.getScreenName();
			}catch(Exception e){
			}
		...
		%>
			...
			%>
				...
				<td class="table-cell <%= columnClassName %>">
					...
					<%
					if(user2 != null && isCustomProfileUrl){
					if (j == 1) { %>
					<a href="<%= customProfileUrl %>"><%=user2.getFirstName() %></a>
						<%}else if(j == 2){ %>
						<a href="<%= customProfileUrl %>"><%=user2.getLastName() %></a>
						<%}else if(j == 3){ %>
						<a href="<%= customProfileUrl %>"><%=user2.getScreenName() %></a>
						<%}else{ 
