...
/**
 * Portlet implementation class ReportsHighChartAction
 */
public class ReportsHighChartAction extends MVCPortlet {

	private static Log _log = LogFactoryUtil.getLog(ReportsHighChartAction.class);
	...
	public void doView(RenderRequest renderRequest, RenderResponse renderResponse) throws IOException, PortletException {
		...
		try {
			...
			for (SPMailCampaign campList : mailCampaignList) {
				...
				rsvpDetailList = RsvpDetailLocalServiceUtil.findByRsvpId(campList.getRsvpId());

				for (RsvpDetail rsvpDetail : rsvpDetailList) {
					try{
						rsvp = RsvpLocalServiceUtil.getRsvp(rsvpDetail.getRsvpId());
		...
		renderRequest.setAttribute("isReport", isReport || SambaashUtil.isSupportRole(themeDisplay.getScopeGroupId(), themeDisplay.getUserId()));
		if(eventsJSONObject.length() > 0){
			renderRequest.setAttribute("eventsJSONObject", eventsJSONObject);
		}else{
			renderRequest.setAttribute("eventsJSONObject", "0");
		}
		if(rsvpJSONObject.length() > 0){
			renderRequest.setAttribute("rsvpJSONObject", rsvpJSONObject);
		}else{
			renderRequest.setAttribute("rsvpJSONObject", "0");
		}
		if(campJSONObject.length() > 0){
			renderRequest.setAttribute("campJSONObject", campJSONObject);
		}else{
			renderRequest.setAttribute("campJSONObject", "0");
		}
		if(assetJSONObject.length() > 0){
			renderRequest.setAttribute("assetJSONObject", assetJSONObject);
		}else{
			renderRequest.setAttribute("assetJSONObject", "0");
		}
	...
	public JSONObject getRsvpCounts(long rsvpId, long campaignId, int mailType) throws PortalException, SystemException {
		int inviteCount = 0;
		int adminCount = 0;
		int selfCount = 0;
		int yesConfirm = 0;
		...
		List<Object[]> linkList = SPMailLinkTrackingLocalServiceUtil.countOpenedMailByLink(campaignId,mailType);
		String link = "";
		String linkUrl = "";
		...
		for (Object[] obj : linkList) {
			...
			//linkJSONObject.put((String) obj[1], ((Long) obj[0]));
			link = (String)obj[1];
			_log.error(String.valueOf(((Long)obj[0])));
			_log.error((String)obj[1]);
...
_log.error("countJSONObject " + countJSONObject.toString());
	...
	public void serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws IOException,
		...
		if (type.equalsIgnoreCase("getMailType")) {
				...
				String campaignId = resourceRequest.getParameter("selectedCampaign");
				...
				try {
					...
					if(mailCampaignEdms.size() > 0){
						for(SPMailCampaignEDM mailCampaignEdm : mailCampaignEdms){
							countJSONObject.put(String.valueOf(mailCampaignEdm.getSeqNo()),mailCampaignEdm.getName());
						}
					}else{
						countJSONObject.put(String.valueOf("-1"),"All");
						countJSONObject.put(String.valueOf("0"),"Main");
						countJSONObject.put(String.valueOf("1"),"Reminder1");
						countJSONObject.put(String.valueOf("2"),"Reminder2");
						countJSONObject.put(String.valueOf("3"),"Reminder3");
					}
