...
<%
	...
	JSONArray assetGlCodes = SystemServiceUtil .getModelList("chartofaccount", "GlCode", groupId, 
			"{\"glCodeType\":\"SubLedger\",\"accountType\":\"Asset\",\"status\":\"Active\"}");
	JSONArray liabilityGlCodes = SystemServiceUtil .getModelList("chartofaccount", "GlCode", groupId, 
			"{\"glCodeType\":\"SubLedger\",\"accountType\":\"Liability\",\"status\":\"Active\"}");
	JSONArray defaultProfitCentreCodes = SystemServiceUtil .getModelList("profitcentre", "ProfitCentreCode", groupId, 
			"{\"status\":\"Active\"}");
	JSONArray settlementGLCode = SystemServiceUtil .getModelList("chartofaccount", "GlCode", groupId, 
			"{\"glCodeType\":\"SubLedger\",\"status\":\"Active\"}");
%>
					...
					<aui:col span="10" cssClass="offset1">
						...
						<aui:row>
							<aui:col span="6" cssClass="formio-component-textfield">
								<div class="form-group">
									...
									<aui:select name="" id="assetSuspenseGlCode"
										cssClass="form-control">
										<aui:option value="null">Select Asset Suspense GL Code...</aui:option>
										<%
											for(int i=0;i<assetGlCodes.length();i++) {
												JSONObject obj = assetGlCodes.getJSONObject(i);
												%><aui:option value='<%=obj.getString("GlCode")%>'><%=obj.getString("GlCode")%></aui:option><%
											}
										%>
									</aui:select>
								</div>
							...
							<aui:col span="6" cssClass="formio-component-textfield">
								<div class="form-group">
									...
									<aui:select name="" id="liabilitySuspenseGlCode"
										cssClass="form-control">
										<aui:option value="null">Select Liability Suspense GL Code...</aui:option>
										<%
											for(int i=0;i<liabilityGlCodes.length();i++) {
												JSONObject obj = liabilityGlCodes.getJSONObject(i);
												%><aui:option value='<%=obj.getString("GlCode")%>'><%=obj.getString("GlCode")%></aui:option><%
											}
										%>
						...
						<aui:row>
							<aui:col span="6" cssClass="formio-component-textfield">
								<div class="form-group">
									...
									<aui:select name="" id="defaultProfitCentre"
										cssClass="form-control">
										<aui:option value="null">Select Default Profit Centre Code...</aui:option>
										<%
											for(int i=0;i<defaultProfitCentreCodes.length();i++) {
												JSONObject obj = defaultProfitCentreCodes.getJSONObject(i);
												%><aui:option value='<%=obj.getString("ProfitCentreCode")%>'><%=obj.getString("ProfitCentreCode")%></aui:option><%
											}
										%>
									</aui:select>
								</div>
											...
											<h3 class="header toggler-header-collapsed" style="position: relative;top: 0;">
												...
												<ul class="nav">
													<li style="float:left"><a href="javascript:void(0);"
														class="btn removeBtn" onclick="RemovePaymentMode(this)"style="top: 0px;padding-left: 16px !important;">Remove</a></li>
														<li><a href="javascript:void(0);" class="minusIcon" style="top: -2px;" onclick="closeSettlementMethod(this)">Remove</a></li>
												</ul>
												...
												<aui:row>
													<aui:row>
														...
														<aui:col span="6" cssClass="formio-component-textfield">
															...
															<aui:select name="" id="settlementGlCodeId"
																cssClass="form-control settlementGlCodeId">
																<aui:option value="null">Select Settlement GL Code...</aui:option>
																<%
																	for(int i=0;i<settlementGLCode.length();i++) {
																		JSONObject obj = settlementGLCode.getJSONObject(i);
																		%><aui:option value='<%=obj.getString("GlCode")%>'><%=obj.getString("GlCode")%></aui:option><%
																	}
																%>
															</aui:select>
						...
						<aui:row>
							<aui:col span="12" cssClass="text-center">
								<aui:button cssClass="btn greenbtn" id="addPaymentModeButton"
									value="ADD PAYMENT MODE" onClick="AddAnotherPaymentMode()"></aui:button>
							</aui:col>
						</aui:row>
