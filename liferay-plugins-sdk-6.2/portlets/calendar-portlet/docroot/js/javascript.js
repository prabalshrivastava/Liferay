				...
				var schedulerEvent = new Liferay.SchedulerEvent(
					{
						...
						eventCreator : calendarBooking.eventCreator
					}
				);
				...
				_doRead: function(options, callback) {
					...
					CalendarUtil.getEvents(
						...
						function(calendarBookings) {
							...
							calendarBookings = AArray.filter(calendarBookings, function(booking) {
								if (Liferay.CalendarUtil.visibleCalendars[booking.calendarId]) {
									return true;
								}
								return false;
							});
					...
					plotCalendarBookings: function(calendarBookings) {
						...
						AArray.each(
							...
							function(item, index) {
								...
								if (Liferay.CalendarUtil.visibleCalendars[calendarId]) {
								}
		...
		var SchedulerEventRecorder = A.Component.create(
			{
				...
				prototype: {
					...
					getTemplateData: function() {
						...
						return A.merge(
							templateData,
							{
								...
								status: schedulerEvent.get('status'),
								eventCreator: schedulerEvent.get('eventCreator'),
								...
								if (permissions.VIEW_BOOKING_DETAILS) {
									...
									CalendarUtil.getCalendarBookingInvitees(
										...
										function(data) {
											...
											instance._syncInviteeContentWithStatus('#' + portletNamespace + 'eventRecorderUsersTd', results.matches);
					...
					_syncInviteeContentWithStatus: function(contentNode, calendarResources) {
						var instance = this;
						contentNode = A.one(contentNode);
						var messageNode = contentNode.one('.calendar-portlet-invitees');
						messageNode.html('');
						var userTpl  = "<div><span>[NAME]</span>&nbsp;&nbsp;&nbsp;<span class='[STATUS_CLASS]'>[STATUS]</span></div>"
						var buffer = "";
						if (calendarResources && calendarResources.length > 0) {
							for (var i = 0  ; i < calendarResources.length ; i++) {
								var user = calendarResources[i];
								buffer = buffer + userTpl.replace("[NAME]", user.name).replace("[STATUS]",user.status).replace('[STATUS_CLASS]','status-' + user.statusNoi18n);
							}
							var users = A.Node.create("<div>" + buffer + "</div>");
							contentNode.html('');
							contentNode.append(users);
						}
					},
