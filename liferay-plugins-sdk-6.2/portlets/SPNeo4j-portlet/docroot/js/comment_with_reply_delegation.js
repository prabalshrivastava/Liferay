function CommentWithReply(param) {
	...
	this.j_comment_delegation = null;
	...
	try{
		...
		for (var i=0; i<that.j_comment_delegation.length; i++) {
			if (navigator.appName == 'Microsoft Internet Explorer') {
				addEventHandler(that.j_comment_delegation[i], "click", commentDelegationClick);
			}else {
				that.j_comment_delegation[i].addEventListener ("click", commentDelegationClick, false);
			}
		}
	...
	function commentDelegationClick(e) {
	try{
		var target = getEventTarget(e);
		var comment_dom_id = target.getAttribute("data-comment-dom-id");
		// document.title += " -comment_dom_id: " + comment_dom_id;
		if (comment_dom_id == "add-comment-textarea") {
			addCommentTextareaFocus(e);
		}
		...
		if (comment_dom_id == "add-comment-form-sumbit-btn") {
			addCommentFormSubmit(e);
		}
		if (comment_dom_id == "delete-link") {
			deleteLinkClick(e);
		}
		if (comment_dom_id == "comment-link") {
			commentLinkClick(e);
		}
	}catch(err) {
		alert(err);
	...
	try{
		var comment_add_textarea = getEventTarget(e);
		expandAddCommentTextarea(comment_add_textarea);
	}catch(err) {
		alert(err);
	}
	}
	...
	try{
		preventDefault(e);
		var comment_link = getEventTarget(e);
		var comment_container = getClosestParentByAttribute(comment_link, "data-comment-dom-id", "comment-container");
		var comment_add_textarea = getFirstElementsByAttribute(comment_container, "textarea", "data-comment-dom-id", "add-comment-textarea");
		var form_container = getClosestParentByAttribute(comment_add_textarea, "data-comment-dom-id", "add-comment-form-container");
		var className = form_container.className ? form_container.className : "" ;
		//If it is already visible, then don't set the styles again.so that blinking effect won't be there when user clicks comment button repeatedly
		if(className.indexOf("hide-content") != -1){
			expandAddCommentTextarea(comment_add_textarea);
			comment_add_textarea.focus();
		}
	...
	function expandAddCommentTextarea(comment_add_textarea) {
		...
		var form_container = getClosestParentByAttribute(comment_add_textarea, "data-comment-dom-id", "add-comment-form-container");
		var formClassName = "";
		if(form_container.className && form_container.className !=null){
			formClassName = form_container.className;
		}
		form_container.className = formClassName.replace("hide-content","");
		...
		comment_add_textarea_container.style.margin = "0 0 0 44px";
		comment_add_textarea_container.style.minHeight = "36px";

		// bind focusout event for current add comment textarea
		if (navigator.appName == 'Microsoft Internet Explorer') {
			addEventHandler(comment_add_textarea, "focusout", addCommentTextareaFocusout);
		}else {
			comment_add_textarea.addEventListener ("blur", addCommentTextareaFocusout, false);
		}
	}
	...
	try{
		...
		form_container.className = formClassName + " hide-content";
		comment_add_textarea.value = "";
	...
	try {
		...
		if (!comment_adding) {
			comment_adding = true;
			var target = getEventTarget(e);
			addComment(getClosestParentByAttribute(target, "data-comment-dom-id", "add-comment-form"));
		}
	}catch(err) {
		alert(err);
	}
	...
	try{
		...
		url += "&class_pk=" + class_pk + "&class_name=" + class_name + "&content=" + encodeURIComponent(content);
	...
	try{
		...
		var nodeContainer = AUI().one("div[data-comment-asset-entity-id="+class_pk+"] .sp-af-comment-composer:not(.hide-content)");
		var comment_entry_container = nodeContainer._node;
		...
		var entry_container = nodeContainer.siblings().one("ul[data-comment-dom-id='entry-container']")._node;
		...
		// update counts
		var parent_count = nodeContainer.siblings().one("span[data-comment-dom-id='parent-count']")._node
	...
	try{
		...
		url += "&class_pk=" + class_pk + "&class_name=" + class_name + "&parent_id=" + parent_id + "&content=" + encodeURIComponent(content);
	...
	try{
		...
		var parent_entry = AUI().one("div[data-comment-asset-entity-id="+class_pk+"] li[data-comment-id="+parent_id+"]")._node;
