...
public class LoginUtil {
	...
	public static long getAuthenticatedUserId(HttpServletRequest request, String login, String password,
			String authType) throws PortalException, SystemException {
		...
		if(_log.isDebugEnabled()){
			_log.debug( " getAuthenticatedUserId : " + company.getCompanyId() + " : " + requestURI  + " : " + contextPath);
		}
		...
		if (requestURI.startsWith(contextPath.concat("/api/liferay"))) {
			...
			if(_log.isDebugEnabled()){
				_log.debug( " authResult : " + authResult + " : login : " + login + " : password length : " +  password.length());
			}
			...
			if(authResult == Authenticator.SUCCESS && !SambaashUtil.hasSiteAccess(themeDisplay, userId)){
	...
	public static void login(HttpServletRequest request, HttpServletResponse response, String login, String password,
		...
		if (PropsValues.SESSION_ENABLE_PHISHING_PROTECTION) {
			...
			String afterLoginSessionId = session.getId();
			SPAuthContext.transferContext(loginSessionId, afterLoginSessionId);
		}
	...
	public static void sendPassword(ActionRequest actionRequest, String fromName, String fromAddress, String toAddress,
	 ...
	 * @throws SystemException
	 * Please check post login action
	 */
	public static void sendWelcomeEmail(User user, String password, ServiceContext serviceContext)
			throws SystemException {
		...
		String mailTemplateIdParameter = SambaashConstants.TEMPLATE_ACCOUNT_CREATED_NOTIFICATION_ID;
		...
		if (Validator.isNull(subject)) {
			subject = PrefsPropsUtil.getContent(user.getCompanyId(), PropsKeys.ADMIN_EMAIL_USER_ADDED_SUBJECT);
		}
