					...
					if (this.get("responseData") !== undefined
						...
						if (this.get("responseData").error) {
							...
							if (rpecStatus === "Rejected") {
								if (getEID("forFinalReview") !== null) {
									// getEID("forFinalReview").style.display =
									// "none";
							...
							if (rpecStatus == "Draft") {
								if (getEID("forFinalReview") !== null) {
									...
									// "not-allowed";
									// getEID("forFinalReview").disabled =
									// "true";
									getEID("forFinalReview").classList
											.add("disable-final-review");
								}
			...
			ajaxCallAPI('POST', 'persist', RPECData, function() {
				...
				if (data.error) {
					...
					if (status == "Approved"
						...
						openSuccessPopup("#success-submit",
								"#success-submit-box", "");
					}
...
function createPanel(panelName, panelId, panelType, isRemovable) {
	if (!isRemovable) {
		...
		if (mode !== 'view'
				&& (userType.toLowerCase() !== 'mentor' && userType
						.toLowerCase() !== 'trainingprincipal')) {
			...
			function() {
				...
				if (panelId === "Nullify") {
					...
					remarksDiv.innerHTML += '<div style="padding:20px" class="remarks-comment"> <textarea type = "text" id="remark_input_'
							+ panelId
							+ '"+ class="remark-log" placeholder = "Log a remark..." style="width: 93%; resize: none;" oninput="addRemarks(event,\''
							+ panelId
...
function propogateRadioButtonsValues() {
	Object
			.keys(competencyObj)
			.forEach(
					function(ck) {
								...
								.forEach(
										function(sk) {
											Object
													...
													.forEach(
															function(qk) {
																var dynamicId = competencyObj[ck][sk][qk].questionId
																		+ "_"
																		+ competencyObj[ck][sk][qk].levelName
																		+ "_"
																		+ competencyObj[ck][sk][qk].score;
																var radioButton = document
																		.getElementById(dynamicId);
																if (radioButton !== undefined
																		&& radioButton !== null) {
...
function refreshProficiencyLevel(tableId) {
	...
	if (oldCompetencyObj[ids[0]] !== undefined
		...
		if (competencyObj[ids[0]][ids[1]][ids[2]] !== undefined) {
			...
			var oldRadioID = questionObj.questionId + '_'
					+ questionObj.levelName + '_' + questionObj.score;
...
function removePanel(panelId, panelName) {
	...
	getEID('panel' + panelId + 'parent').remove();
						...
						YUI()
								.use(
										...
										function(Y) {
											...
											Y
													.one(
															'.popup-back-to-record-submit-review-reject')
													.on(
															'click',
															function() {
																// window.history.back();
											...
											Y
													.one(
															'.popup-back-to-record-submit-review')
													.on(
															'click',
															function() {
																// window.history.back();
											...
											Y
													.one(
															'.popup-back-to-home-submit-review')
													.on(
															'click',
															function() {
																window.location.href = dashBoardLink;
											...
											Y
													.one(
															'.popup-back-to-home-submit-review-reject')
													.on(
															'click',
															function() {
																window.location.href = dashBoardLink;
											...
											Y.one('.popup-ok').on('click',
													function() {
														modal.hide();
													});
											Y
													.one('.popup-remarks-yes')
													.on(
															'click',
															function() {
																modal.hide();
