...
public class EditPermissionsAction extends PortletAction {
	...
	protected void updateRolePermissions(ActionRequest actionRequest)
		...
		// Nov 4th, 2015 Naresh - Added below code to inherit permissions
		 if(DLFolder.class.getName().equalsIgnoreCase(selResource)){
			 try {
				 // comma separated role ids - permissions got changed for these role ids
				 String temp = ParamUtil.getString(actionRequest,"modifiedRoleIds");
				 boolean propagateToChilds = ParamUtil.getBoolean(actionRequest,"propagateToChilds");
				 if(_log.isDebugEnabled()){
					 _log.debug(" modifiedRoleIds = " + temp + " propagateToChilds = " + propagateToChilds);
				 }
				 if(Validator.isNotNull(temp) && propagateToChilds){
					 String []stArr = temp.split(StringPool.COMMA);
					 
					 long rolIdsArray[] = GetterUtil.getLongValues(stArr);
					 
					 List<Long>roleIdList = ListUtil.toList(rolIdsArray);
					 if(roleIdList.size() > 0){
						 _log.debug(" before call to PermissionUtil.inheritPermissionsToChilds. resourcePrimKey  " + resourcePrimKey + ", roleIdList " + roleIdList);
						 
						 PermissionUtil.inheritPermissionsToChilds(GetterUtil.getLong(resourcePrimKey),themeDisplay.getScopeGroupId(),
								 themeDisplay.getCompanyId(),SambaashConstants.PERMISSION_OPERATION_SET,false,roleIdList);
					 }
				 }
	...
	private static Log _log = LogFactoryUtil.getLog(EditPermissionsAction.class.getName());
}