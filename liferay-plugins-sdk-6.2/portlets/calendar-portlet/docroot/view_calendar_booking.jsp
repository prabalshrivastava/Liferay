...
<aui:fieldset>
	<dl class="property-list inline-block">
		...
		<%
		List<CalendarBooking> childCalendarBookings = calendarBooking.getParentCalendarBooking().getChildCalendarBookings();
		%>
		...
		<c:if test="<%= !childCalendarBookings.isEmpty() %>">
			...
			<dd class="inline-block">
				...
				<%
				String tpl = "<div><span>[NAME]</span>&nbsp;&nbsp;&nbsp;<span class='[STATUS_CLASS]'>[STATUS]</span></div>";
				StringBuilder sb = new StringBuilder();
				for (CalendarBooking childCalendarBooking : childCalendarBookings) {
					CalendarResource calendarResource = childCalendarBooking.getCalendarResource();
					String name = GetterUtil.getString(calendarResource.getName());
					String[] status = CalendarUtil.getStatusStrings(themeDisplay.getLocale(), childCalendarBooking.getStatus());
					sb.append(StringUtil.replace(tpl, new String[]{"[NAME]","[STATUS]","[STATUS_CLASS]",}, new String[]{name,status[0],"status-"+status[1]}));
