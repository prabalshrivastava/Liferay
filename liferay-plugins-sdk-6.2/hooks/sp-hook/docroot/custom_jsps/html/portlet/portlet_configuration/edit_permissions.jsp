...
<div class="edit-permissions">
	<aui:form action="<%= actionPortletURL.toString() %>" method="post" name="fm">
		...
		<aui:input name="modifiedRoleIds"  type="hidden"  />
		<aui:input name="propagateToChilds"  type="hidden"  />
		<c:choose>
...
<%
 if(com.sambaash.platform.util.PermissionUtil.isInheritPermissionModelEnabled() && 
		 (DLFileEntry.class.getName().equalsIgnoreCase(modelResource) || DLFolder.class.getName().equalsIgnoreCase(modelResource))){
	 boolean isFolder = DLFolder.class.getName().equalsIgnoreCase(modelResource);
%>
<script src="/html/js/sp/resourcePerms.js">
</script>
<script>
var configObj = {
