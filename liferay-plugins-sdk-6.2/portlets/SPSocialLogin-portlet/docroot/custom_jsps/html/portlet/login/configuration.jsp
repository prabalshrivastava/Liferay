...
List<AssetVocabulary> assetVocabularies = AssetVocabularyLocalServiceUtil.getCompanyVocabularies(themeDisplay.getCompanyId());
String spSiteAuthVocId = portletPreferences.getValue("spSiteAuthVocId", "0");
List<AssetCategory> categories = Collections.emptyList();
if (spSiteAuthVocId != null && !spSiteAuthVocId.equals("")) {
	categories = AssetCategoryLocalServiceUtil.getVocabularyCategories(Long.parseLong(spSiteAuthVocId), -1, -1, null);
}
String spSiteAuth = portletPreferences.getValue("spSiteAuth", "0");
try {
	Long.parseLong(spSiteAuth);
} catch(Exception e) {
	spSiteAuth = "0";
}
%>
	...
	<c:choose>
		...
		<c:otherwise>
			<aui:fieldset>
				...
				<aui:select name="preferences--spSiteAuthVocId--"
					label="sp-label-select-spsite-auth-vocabulary"
					onChange='<%= renderResponse.getNamespace() + "updateSpSiteAuthList(this);" %>'>
					<aui:option value="0" label="Please Select" 
							selected="<%= Long.parseLong(spSiteAuth) <= 0 %>" />
					<% 
					for(AssetVocabulary assetVocabulary: assetVocabularies)
					{
					%>
						<aui:option value="<%= assetVocabulary.getVocabularyId() %>"
							selected="<%= assetVocabulary.getVocabularyId() == Long.parseLong(spSiteAuthVocId) %>"
							label="<%= assetVocabulary.getName() %>" />					
					<% 
					}
					%>
				</aui:select>
				<aui:select name="preferences--spSiteAuth--"
					label="sp-label-select-spsite-auth">
					<aui:option value="0" label="Please Select" 
							selected="<%= Long.parseLong(spSiteAuth) <= 0 %>" />
					<% 
					...
					%>
						<aui:option value="<%= category.getCategoryId() %>"
							selected="<%= category.getCategoryId() == Long.parseLong(spSiteAuth) %>"
			...
			<aui:script>
			function <portlet:namespace />updateSpSiteAuthList() {
				var A = AUI();
				var sourceElement = A.one("#<portlet:namespace />spSiteAuthVocId");
				var targetElement = A.one("#<portlet:namespace />spSiteAuth");
			    console.log("get category => "+sourceElement.val());
				Liferay.Service(
				  '/SPMicroservice-portlet.spmicroservice/get-vocabulary-categories',
				  {
				    vocabularyId: sourceElement.val(),
				    start: -1,
				    end: -1
				  },
				  function(categories) {
				    targetElement.html("");
                    targetElement.append("<option value='0'>Please Select</option>");
				    for (var j=0; j < categories.length; j++) {
                      targetElement.append("<option value='" + categories[j].categoryId + "'>" + categories[j].name + "</option>");
                    }
				  }
				);
			}
