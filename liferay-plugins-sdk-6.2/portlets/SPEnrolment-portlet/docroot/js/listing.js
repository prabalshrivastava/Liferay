...
function storeEnrolmentIdsInSessionAndReturnKey() {
	var _sessionKey = new Date().getTime() + "" + SambaashUtils.getRandomInt(1000);
	if (sessionStorage) {
		sessionStorage.setItem(_sessionKey,JSON.stringify(enrolmentIds));
	} else { // if web storage not supported by browser, fallback to cookie (no choice)
		setCookie(_sessionKey, JSON.stringify(enrolmentIds));
	}
	return _sessionKey;
}
