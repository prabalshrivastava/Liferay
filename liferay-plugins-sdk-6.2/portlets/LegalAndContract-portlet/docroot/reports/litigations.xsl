...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
		...
		<fo:root>
			...
			<fo:page-sequence master-reference="A4-landspace">
				...
				<fo:flow flow-name="xsl-region-body">
					<fo:block>
						<xsl:apply-templates select="/litigationList" />
					</fo:block>
					<fo:block id="theEnd" />
				</fo:flow>
			</fo:page-sequence>
	...
	<xsl:template match="/litigationList">
		<fo:block>
			<fo:block font-size="9pt">
				<fo:table border="solid" border-after-width.length="1pt"
   					 border-after-width.conditionality="retain">
					<fo:table-header>
						<fo:table-row border="solid">
							<xsl:for-each select="*[2]/dataMap/entry/key">
								<xsl:if test="not(starts-with(text(), 'DetailsLink'))">
						...
						<xsl:for-each select="*/dataMap">
							<xsl:choose>
								<xsl:when test="position() mod 2 != 0">
									<fo:table-row>
										<xsl:for-each select="*">
											<xsl:if test="not(starts-with(key/text(), 'DetailsLink'))">
	...
	<xsl:template name="cell">
		<fo:block linefeed-treatment="preserve" xsl:use-attribute-sets="fontFamily">
			<xsl:choose>
				<xsl:when test="key = 'Trademark Application Number / Country'">
					<xsl:variable name="regNo" select="value"></xsl:variable>
				...
				<xsl:when test="key = 'Trademark'">
					...
					<xsl:variable name="link" select="..//entry[last() - 1]//value"></xsl:variable>
					<fo:basic-link external-destination="url({$link})"
						color="blue">
					<xsl:choose>
