...
public class UploadHelper {
	...
	private static Log _log = LogFactoryUtil.getLog(UploadHelper.class.getName());

	private final String DEFAULT_ROOT = "Activity Hub Content";
	
	final protected long scopeGroupId;
	final protected long companyId;
	
	public UploadHelper(long scopeGroupId, long companyId){
		this.scopeGroupId = scopeGroupId;
		this.companyId = companyId;
	}
	
	public String getPEFolderName(){
		String name = SambaashUtil.getParameter(SPActivityHubConstants.SP_PARAM_ACTIVITYHUB_ROOT_FOLDR, 0);
	...
	public Folder getApplicationsFolder() throws PortalException, SystemException{
		Folder root = getRootFolder();
		Folder temp = getAdminFolder(root.getFolderId(), "activityhub");
		return temp;
	}
	...
	public Folder getFolder(long parentFolderId, String folderName) throws PortalException, SystemException{
		...
		try {
			...
			folder = DLAppServiceUtil.addFolder(scopeGroupId, parentFolderId,
	...
	/**
	 *  Admin folders are high previliged folders and must be created by admin as owner.
	 *  
	 *  Below is the folder structure. (root ,temp and applications are admin folders)
	 *  
	 *  Process Engine Content 
	 *     -- Temp
	 *     -- Applications
	 *           --68283 (sub folders-named with process state id)
	 *           --83473
	...
	private String[] getDefaultRoles() {
		...
		try {
			String[] ADMIN_DEFAULT_ROLE_NAMES = StringUtil.splitLines(PropsUtil.get(PropsKeys.ADMIN_DEFAULT_ROLE_NAMES));
			defaultRoleNames = PrefsPropsUtil.getStringArray(
	...
	public  void addDefaultFolderPermissions(long folderId){
		...
		String []privilegedRoles = getDefaultRoles();
		String actions[] = {ActionKeys.ADD_DOCUMENT,ActionKeys.ADD_SUBFOLDER,
	...
	public void moveFile(long fileEntryId,long destFolderId) throws PortalException, SystemException{
		...
		try{
			...
			DLAppServiceUtil.updateFileEntry(
					fileEntryId,
					newName, dfe.getMimeType(),
					newName, "Renamed from " + dfe.getTitle() + " to " + newName, "", true, dfe.getContentStream(),
					dfe.getSize(), serviceContext);
			DLAppServiceUtil.moveFileEntry(fileEntryId, destFolderId, serviceContext);
		}
	}
	
	public FileEntry storeFile(File file, long destFolderId,String destFileName) throws PortalException, SystemException, IOException{
		ServiceContext serviceContext = new ServiceContext();
		FileEntry dfe = null ;
		String mimeType = Files.probeContentType(file.toPath());
		//BufferedInputStream bis = new BufferedInputStream(new FileInputStream(file));
		try{
			dfe = DLAppServiceUtil.addFileEntry(scopeGroupId, destFolderId, destFileName, mimeType, destFileName, "Copied from " + file.getAbsolutePath(), StringPool.BLANK, file, serviceContext);
		}catch(DuplicateFileException fe){
			...
			dfe = DLAppServiceUtil.addFileEntry(scopeGroupId, destFolderId, newName, mimeType, newName, "Copied from " + file.getAbsolutePath(), StringPool.BLANK, file, serviceContext);
		}
		addDefaultFilePermissions(dfe.getFileEntryId());
		return dfe;
	}
	
	public static File download(String url) throws PortalException, SystemException, MalformedURLException, IOException{
		File temp = FileUtil.createTempFile();
		FileUtils.copyURLToFile(new URL(url), temp);
		return temp;
	}
	public static File download(String url,String destName) throws PortalException, SystemException, MalformedURLException, IOException{
	...
	/*public File download(String url,String fileName) throws PortalException, SystemException, MalformedURLException, IOException{
		
		String path = System.getProperty("java.io.tmpdir") + File.separator + fileName;
		File file = new File(path);
		if(file.exists()){
			String tempName = generateUniqueName(fileName);
			path = System.getProperty("java.io.tmpdir") + File.separator + tempName;
			file = new File(path);
		}
		FileUtils.copyURLToFile(new URL(url), file);
		return file;
