...
import com.sambaash.platform.util.SPFileUtils;
...
public class EditFileEntryAction extends PortletAction {
	...
	private String getParentPath(String selectedFileName){
		String parentPath = StringPool.BLANK;
		if(selectedFileName.indexOf(StringPool.FORWARD_SLASH) != -1){
			String separator = selectedFileName.startsWith(StringPool.FORWARD_SLASH) ? StringPool.BLANK : StringPool.FORWARD_SLASH;
			parentPath = _TEMP_FOLDER_NAME + separator + selectedFileName.substring(0,selectedFileName.lastIndexOf(StringPool.FORWARD_SLASH));
		}else{
			parentPath = _TEMP_FOLDER_NAME;
		}
		return parentPath;
	}
	...
	private String getFileNameFromPath(String path){
		String file = path;
		if(path.indexOf(StringPool.FORWARD_SLASH) != -1){
			file = path.substring(path.lastIndexOf(StringPool.FORWARD_SLASH) + 1);
		}
		return file;
	}
	...
	protected void addMultipleFileEntries(
		...
		String parentPath = getParentPath(selectedFileName);
		selectedFileName =  getFileNameFromPath(selectedFileName);
		if(originalSelectedFileName.indexOf(StringPool.FORWARD_SLASH) != -1){
			String []ancesotrs =  parentPath.split(StringPool.FORWARD_SLASH);//ArrayUtil.subset(pathElements, 1, pathElements.length-1);
			  try{
				  	long parent = folderId;
					ServiceContext serviceContextFolder = ServiceContextFactory.getInstance(
							DLFolder.class.getName(), actionRequest);
						String[] groupPermissions = new String[]{ActionKeys.ADD_DOCUMENT, ActionKeys.ADD_SHORTCUT, ActionKeys.ADD_SUBFOLDER, ActionKeys.VIEW};
						String[] guestPermissions = new String[]{ActionKeys.VIEW};
						serviceContextFolder.setGroupPermissions(groupPermissions);
						serviceContextFolder.setGuestPermissions(guestPermissions);
						// i=1 bcz, ignore the temp folder
				  	for(int i = 1; i < ancesotrs.length ; i++){
				  		try{
				  			Folder folder = SPFileUtils.getOrAddFolder(repositoryId, parent, ancesotrs[i], "", serviceContextFolder);
				  	...
				  	// save the file to dragged folder.
				  	folderId = parent;
			  }catch(Exception ex){
				  ex.printStackTrace();
				  _log.error(ex);
	...
	protected void addTempFileEntry(
		...
		String fullPath = ParamUtil.getString(uploadPortletRequest,"fullPath");
		
		String parent  = StringPool.BLANK;
		if(Validator.isNotNull(fullPath)){
			 int parentPathEnd = fullPath.lastIndexOf(StringPool.FORWARD_SLASH);
			 if(parentPathEnd != -1){
				 parent = fullPath.substring(0,parentPathEnd);
			 }
		}
		...
		try {
			...
			String tempFolder = _TEMP_FOLDER_NAME;
			if(Validator.isNotNull(parent)){
				if(parent.startsWith("/")){
					tempFolder =  tempFolder + parent;
				}else{
					tempFolder =  tempFolder + StringPool.FORWARD_SLASH + parent;
				}
			}

			DLAppServiceUtil.addTempFileEntry(
				themeDisplay.getScopeGroupId(), folderId, sourceFileName,
				tempFolder, inputStream, contentType);

			JSONObject jsonObject = JSONFactoryUtil.createJSONObject();

			if(Validator.isNotNull(parent)){
				jsonObject.put("name", parent + StringPool.FORWARD_SLASH + sourceFileName);
	...
	protected void deleteTempFileEntry(
		...
		long folderId = ParamUtil.getLong(actionRequest, "folderId");
		// fileName can also represent file absolute path, if folder is being uploaded
		String fileName = ParamUtil.getString(actionRequest, "fileName");
		String folderName = getParentPath(fileName);
