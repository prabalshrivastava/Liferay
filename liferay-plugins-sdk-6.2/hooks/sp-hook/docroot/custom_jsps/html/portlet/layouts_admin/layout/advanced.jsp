...
<%@ page import="com.sambaash.platform.srv.spservices.model.SPSiteSetup" %>
<%@ page import="com.sambaash.platform.srv.spservices.service.SPSiteSetupLocalServiceUtil" %>
...
<aui:fieldset>
	...
	String subProduct = GetterUtil.getString(layoutTypeSettings.getProperty("subProduct"));
	...
	<aui:select label="Sub-Product" name="TypeSettingsProperties--subProduct--" multiple="true">

		<%
		
		List<SPSiteSetup> spSiteSetupList = SPSiteSetupLocalServiceUtil.getSPSiteSetups(-1, -1);
		for (SPSiteSetup spSiteSetup : spSiteSetupList) {
		%>
			<aui:option label='<%= spSiteSetup.getSubProductName() %>' selected="<%= subProduct.contains(String.valueOf(spSiteSetup.getSubProductId())) %>" value="<%= String.valueOf(spSiteSetup.getSubProductId())%>" />
		<%
		}
		%>
