...
import com.sambaash.platform.util.ScssUtil;
...
public class EditLayoutSetAction extends EditLayoutsAction {
	...
	protected void updateLayoutSet(ActionRequest actionRequest,
		...
		updateLookAndFeel(actionRequest, themeDisplay.getCompanyId(),
				...
				layoutSet.getThemeId(), layoutSet.getSettingsProperties());

		updateMergePages(actionRequest, liveGroupId);

		updateSettings(actionRequest, liveGroupId, stagingGroupId,
	...
	protected void updateLogo(ActionRequest actionRequest, long liveGroupId,
		...
		try {
			...
			LayoutSetServiceUtil.updateLogo(groupId, privateLayout, useLogo,
					inputStream, false);
		} finally {
	...
	protected void updateLookAndFeel(ActionRequest actionRequest,
		...
		for (String device : devices) {
			...
			LayoutSetServiceUtil.updateLookAndFeel(groupId, privateLayout,
					deviceThemeId, deviceColorSchemeId, deviceCss,
					deviceWapTheme);
	...
	protected void updateMergePages(ActionRequest actionRequest,
			long liveGroupId) throws Exception {

		boolean mergeGuestPublicPages = ParamUtil.getBoolean(actionRequest,
		...
		typeSettingsProperties.setProperty("mergeGuestPublicPages",
				String.valueOf(mergeGuestPublicPages));
	...
	protected void updateSettings(ActionRequest actionRequest,
			long liveGroupId, long stagingGroupId, boolean privateLayout,
			UnicodeProperties settingsProperties) throws Exception {
		...
		LayoutSetServiceUtil.updateSettings(groupId, privateLayout,
