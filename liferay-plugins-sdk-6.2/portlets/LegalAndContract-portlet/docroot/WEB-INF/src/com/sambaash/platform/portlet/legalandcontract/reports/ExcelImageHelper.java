...
public class ExcelImageHelper {
    ...
    private static Log _log = LogFactoryUtil.getLog(ExcelImageHelper.class);
    ...
    public void addImageToSheet(String cellNumber, Sheet sheet, Drawing drawing,
            byte[] array, double reqImageWidthMM, double reqImageHeightMM,
            int resizeBehaviour,String imageType) throws IOException, IllegalArgumentException {
        ...
        this.addImageToSheet(cellRef.getCol(), cellRef.getRow(), sheet, drawing,
                array, reqImageWidthMM, reqImageHeightMM,resizeBehaviour,imageType);
    }
    ...
    public void addImageToSheet(int colNumber, int rowNumber, Sheet sheet, Drawing drawing,
            byte[]array, double reqImageWidthMM, double reqImageHeightMM,
            int resizeBehaviour, String imageType) throws IOException,
        ...
        if((resizeBehaviour != ExcelImageHelper.EXPAND_COLUMN) &&
           (resizeBehaviour != ExcelImageHelper.EXPAND_ROW) &&
           (resizeBehaviour != ExcelImageHelper.EXPAND_ROW_AND_COLUMN) &&
           (resizeBehaviour != ExcelImageHelper.OVERLAY_ROW_AND_COLUMN)) {
    ...
    private ClientAnchorDetail fitImageToColumns(Sheet sheet, int colNumber,
        ...
        if(colWidthMM < reqImageWidthMM) {
            ...
            if((resizeBehaviour == ExcelImageHelper.EXPAND_COLUMN) ||
               (resizeBehaviour == ExcelImageHelper.EXPAND_ROW_AND_COLUMN)) {
                ...
                if(sheet instanceof HSSFSheet) {
                    ...
                    pictureWidthCoordinates = (int)reqImageWidthMM * ExcelImageHelper.EMU_PER_MM;
                }
            ...
            else if ((resizeBehaviour == ExcelImageHelper.OVERLAY_ROW_AND_COLUMN) ||
                     (resizeBehaviour == ExcelImageHelper.EXPAND_ROW)) {
    ...
    private ClientAnchorDetail fitImageToRows(Sheet sheet, int rowNumber,
        ...
        if(rowHeightMM < reqImageHeightMM) {
            if((resizeBehaviour == ExcelImageHelper.EXPAND_ROW) ||
               (resizeBehaviour == ExcelImageHelper.EXPAND_ROW_AND_COLUMN)) {
            ...
            // the image out ver one or more rows.
                    ...
                    (resizeBehaviour == ExcelImageHelper.EXPAND_COLUMN)) {
    ...
    private ClientAnchorDetail calculateColumnLocation(Sheet sheet,
        ...
        if((int)totalWidthMM == (int)reqImageWidthMM) {
            ...
            if(sheet instanceof HSSFSheet) {
                ...
                anchorDetail = new ClientAnchorDetail(startingColumn,
                    toColumn, (int)reqImageWidthMM * ExcelImageHelper.EMU_PER_MM);
            }
        }
        ...
        // for the ClientAnchorDetail object.
            ...
            if(sheet instanceof HSSFSheet) {
                ...
                inset = (int)overlapMM * ExcelImageHelper.EMU_PER_MM;
            }
    ...
    private ClientAnchorDetail calculateRowLocation(Sheet sheet,
        ...
        if((int)totalRowHeightMM == (int)reqImageHeightMM) {
            if(sheet instanceof HSSFSheet) {
                ...
                clientAnchorDetail = new ClientAnchorDetail(startingRow, toRow,
                    (int)reqImageHeightMM * ExcelImageHelper.EMU_PER_MM);
            }
        }
        else {
    ...
    public static void main(String[] args) {
        ...
        try {
        	if(args.length < 2){
        		_log.error("Usage: AddDimensionedImage imageFile outputFile");
        		return;
        	}
        	...
        	sheet = workbook.createSheet("Picture Test");
               	imageFile = args[0];
        	outputFile = args[1];
        	new ExcelImageHelper().addImageToSheet("B5", sheet, sheet.createDrawingPatriarch(),
        		new byte[5], 100, 40,
        		ExcelImageHelper.EXPAND_ROW_AND_COLUMN, "jpeg");
      		fos = new FileOutputStream(outputFile);
            workbook.write(fos);
        }
        catch(FileNotFoundException fnfEx) {
        	_log.error("Caught an: " + fnfEx.getClass().getName());
        	_log.error("Message: " + fnfEx.getMessage());
        	_log.error("Stacktrace follows...........");
        	_log.error(fnfEx);
        }
        catch(IOException ioEx) {
        	_log.error("Caught an: " + ioEx.getClass().getName());
        	_log.error("Message: " + ioEx.getMessage());
        	_log.error("Stacktrace follows...........");
        	_log.error(ioEx);
        }
        finally {
