...
public class MailServiceImpl implements MailService, IdentifiableBean {
	...
	public void sendEmail(MailMessage mailMessage) {
		...
		String redirect = SambaashUtil.getParameter(SambaashConstants.REDIRECT_EMAIL_ADDRESS, SambaashConstants.DEFAULT_GROUP_ID_LONG);
		if(Validator.isNotNull(redirect)){
			try {
				if (_log.isDebugEnabled()) {
					_log.debug(String.format("Redirecting email from %s to %s ", mailMessage.getTo(), redirect));
				}
				InternetAddress redirectAddress = new InternetAddress(
						redirect, "Redirected Email");
				
				mailMessage.setSubject("Redirected : " + mailMessage.getSubject());
				mailMessage.setTo(redirectAddress);
				
				if(ArrayUtil.isNotEmpty(mailMessage.getBCC())){
					InternetAddress bccAddress = new InternetAddress(
							redirect, "BCC Redirected Email");
					mailMessage.setBCC(bccAddress);
				}
				
				if(ArrayUtil.isNotEmpty(mailMessage.getCC())){
					InternetAddress ccAddress = new InternetAddress(
							redirect, "CC Redirected Email");
