...
public class NtlmFilter extends BasePortalFilter {
	...
	public boolean isFilterEnabled(
		...
		try {
			...
			// Naresh - 29-Oct-2014 - Removed the condition BrowserSnifferUtil.isIe(request) so that
			//  users can login into system from other browsers.
	...
	protected void processFilter(
		...
		//TODO: added for testing purpose. have to remove
		String sendNtlmHeader = ParamUtil.getString(request, "resendntlmheader");
		if("resend".equalsIgnoreCase(sendNtlmHeader)){
		...
		if (authorization.startsWith("NTLM")) {
			...
			_log.debug("Ntlm Manager " + ntlmManager);
			...
			if (serverChallenge == null) {
				_log.debug("Server challenge is null. so SC_UNAUTHORIZED (401) header");
				response.setContentLength(0);
				response.setHeader(HttpHeaders.WWW_AUTHENTICATE, "NTLM");
				response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
			...
			try {
				_log.debug("Calling ntlmManager.authenticate");
				...
				if (_log.isErrorEnabled()) {
					...
					// just to save log size just printing the message
					_log.error("Unable to perform NTLM authentication " + e.getMessage());
				}
			}
			finally {
			...
			if (ntlmUserAccount == null) {
				_log.debug("ntlmUserAccount is null");
				response.setContentLength(0);
				response.setHeader(HttpHeaders.WWW_AUTHENTICATE, "NTLM");
				response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
				...
				//response.sendRedirect("/signin?redirect="+ HttpUtil.encodeURL(PortalUtil.getCurrentURL(request)));
			...
			_log.debug("ntlm user successfully authenticated");
			...
			if (ntlmUserAccount == null) {
				_log.debug("ntlmUserAccount is null. so sending ntlm header 401");
				response.setContentLength(0);
				response.setHeader(HttpHeaders.WWW_AUTHENTICATE, "NTLM");
				response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
