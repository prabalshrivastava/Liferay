...
public class UserLocalServiceImpl extends UserLocalServiceBaseImpl {
	...
	public User addUserWithWorkflow(
		...
		if (Validator.isNull(workflowServiceContext)) {
	...
	public void completeUserRegistration(
		...
		if (company.isStrangersVerify()) {
			//do not send any emails if send email is false - Gaurav
			if(sendEmail){
	...
	public void sendEmailAddressVerification(
		...
		try {
			...
			long subProductId = -1L;
			
			if (Validator.isNotNull(serviceContext.getAttribute("subProductId"))) {
				subProductId = ((Long) serviceContext.getAttribute("subProductId")).longValue();
			}

			if (Validator.isNotNull(serviceContext.getAttribute("userType"))) {
				userType = (String) serviceContext.getAttribute("userType");
			}
			...
			if (subProductId < 0) {
				subProductId = SambaashUtil.getSubProductId(serviceContext.getRequest());
			}


			_log.error(String.format("From service context request PortalURL %s, getPathMain %s, subProductId %s",
					serviceContext.getPortalURL(), serviceContext.getPathMain(), subProductId));
			...
			for (SPSiteSetup spSiteSetup : SambaashUtil.emptyIfNull(spSiteSetupList)) {
				...
				if (spUserTypeConfig != null) {
					SPMailTemplate template = SPMailTemplateServiceUtil.getTemplateByName(spSiteSetup.getProductId(),
							spSiteSetup.getSubProductId(), spUserTypeConfig.getEmailVerificationTemplateName());
					if (Validator.isNotNull(template)) {
						...
						fromName = SambaashUtil.getEmailFromName(PortalUtil.getDefaultCompanyId(), template);
						fromAddress =  SambaashUtil.getEmailFromAddress(PortalUtil.getDefaultCompanyId(), template);
					}
				}
			...
			if (_log.isDebugEnabled()) {
				_log.debug("Customised verification template not found, will use default template.");
			}
		}
		...
		if (Validator.isNull(fromName)) {
			fromName = PrefsPropsUtil.getString(user.getCompanyId(), PropsKeys.ADMIN_EMAIL_FROM_NAME);
		}

		if (Validator.isNull(fromAddress)) {
			fromAddress = PrefsPropsUtil.getString(user.getCompanyId(), PropsKeys.ADMIN_EMAIL_FROM_ADDRESS);
		}
	...
	public void verifyEmailAddress(String ticketKey)
		...
		/** Added By Gaurav for updating userRegistrationStatus to Active after verification is done **/
		SocialProfile socialProfile = SocialProfileLocalServiceUtil.getSocialProfile(user.getUserId());
		socialProfile.setUserRegistrationStatus(REGISTRATION.USER_REGISTRATION_STATUS_ACTIVE);
	...
	protected void validateScreenName(
		...
		// June 6 2016, Naresh - Added below logic to allow underline in screen name
		String temp = screenName.replace(StringPool.UNDERLINE, StringPool.BLANK);
		if (!screenNameValidator.validate(companyId, temp)) {
			throw new UserScreenNameException();
		}
