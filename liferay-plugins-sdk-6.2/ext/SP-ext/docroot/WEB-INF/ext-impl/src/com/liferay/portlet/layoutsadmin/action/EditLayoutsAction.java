...
public class EditLayoutsAction extends PortletAction {
	...
	public void processAction(ActionMapping actionMapping,
		...
		String closeRedirect = ParamUtil.getString(actionRequest,
				"closeRedirect");
		...
		try {
			...
			if (cmd.equals(Constants.ADD) || cmd.equals(Constants.UPDATE)) {
				Object[] returnValue = updateLayout(actionRequest,
						actionResponse);

				layout = (Layout) returnValue[0];
				oldFriendlyURL = (String) returnValue[1];

				redirect = updateCloseRedirect(themeDisplay, redirect, null,
						layout, oldFriendlyURL);
				closeRedirect = updateCloseRedirect(themeDisplay,
				...
				if (plid <= 0) {
					...
					boolean privateLayout = ParamUtil.getBoolean(actionRequest,
							"privateLayout");
					long layoutId = ParamUtil
							.getLong(actionRequest, "layoutId");

					layout = LayoutLocalServiceUtil.getLayout(groupId,
							privateLayout, layoutId);

					plid = layout.getPlid();
				}

				Object[] returnValue = SitesUtil.deleteLayout(actionRequest,
						actionResponse);
				...
				redirect = updateCloseRedirect(themeDisplay, redirect, group,
						null, oldFriendlyURL);

				long refererPlid = themeDisplay.getRefererPlid();

				if (plid == refererPlid) {
					redirect = HttpUtil.setParameter(redirect, "refererPlid",
							newRefererPlid);
					redirect = HttpUtil.setParameter(redirect,
							actionResponse.getNamespace() + "selPlid", 0);
				}
			...
			sendRedirect(portletConfig, actionRequest, actionResponse,
	...
	public ActionForward render(ActionMapping actionMapping,
		...
		try {
			...
			SessionErrors
					.add(renderRequest, PrincipalException.class.getName());
	...
	@Override
	public void serveResource(ActionMapping actionMapping,
			ActionForm actionForm, PortletConfig portletConfig,
	...
	protected void checkPermission(PermissionChecker permissionChecker,
			Group group, Layout layout, long selPlid) throws PortalException,
			SystemException {

		if (selPlid > 0) {
			LayoutPermissionUtil.check(permissionChecker, layout,
					ActionKeys.VIEW);
		} else {
			GroupPermissionUtil
					.check(permissionChecker, group, ActionKeys.VIEW);
		}
	}
	...
	protected void checkPermissions(PortletRequest portletRequest)
		...
		if (cmd.equals(Constants.ADD)) {
			...
			if (parentPlid == LayoutConstants.DEFAULT_PARENT_LAYOUT_ID) {
				if (!GroupPermissionUtil.contains(permissionChecker,
				...
				if (!LayoutPermissionUtil.contains(permissionChecker, layout,
			...
			if (group.isCompany()) {
				...
				LayoutPrototypePermissionUtil.check(permissionChecker,
						group.getClassPK(), ActionKeys.UPDATE);
			} else if (group.isLayoutSetPrototype()) {
				LayoutSetPrototypePermissionUtil.check(permissionChecker,
						group.getClassPK(), ActionKeys.UPDATE);
				...
				UserPermissionUtil.check(permissionChecker, groupUserId,
						organizationIds, ActionKeys.UPDATE);
			} else {
	...
	protected void deleteLayoutRevision(ActionRequest actionRequest)
		...
		long layoutRevisionId = ParamUtil.getLong(actionRequest,
				"layoutRevisionId");

		LayoutRevision layoutRevision = LayoutRevisionLocalServiceUtil
				.getLayoutRevision(layoutRevisionId);
		...
		if (updateRecentLayoutRevisionId) {
			StagingUtil.setRecentLayoutRevisionId(request,
	...
	protected void enableLayout(ActionRequest actionRequest) throws Exception {
		long incompleteLayoutRevisionId = ParamUtil.getLong(actionRequest,
		...
		long layoutBranchId = ParamUtil.getLong(actionRequest,
				"layoutBranchId", incompleteLayoutRevision.getLayoutBranchId());
	...
	protected String getColorSchemeId(long companyId, String themeId,
			String colorSchemeId, boolean wapTheme) throws Exception {
	...
	protected String getDefaultThemeSetting(Layout layout, String key,
			String device, boolean inheritLookAndFeel) {
	...
	protected void inheritMobileRuleGroups(Layout layout,
		...
		List<MDRRuleGroupInstance> parentMDRRuleGroupInstances = MDRRuleGroupInstanceLocalServiceUtil
				.getRuleGroupInstances(Layout.class.getName(),
						layout.getParentPlid());

		for (MDRRuleGroupInstance parentMDRRuleGroupInstance : parentMDRRuleGroupInstances) {

			MDRRuleGroupInstance mdrRuleGroupInstance = MDRRuleGroupInstanceServiceUtil
					.addRuleGroupInstance(layout.getGroupId(),
			...
			List<MDRAction> parentMDRActions = MDRActionLocalServiceUtil
					.getActions(parentMDRRuleGroupInstance
	...
	protected void resetMergeFailCountAndMerge(ActionRequest actionRequest)
		...
		long layoutPrototypeId = ParamUtil.getLong(actionRequest,
				"layoutPrototypeId");
		...
		layoutPrototype = LayoutPrototypeServiceUtil
				.getLayoutPrototype(layoutPrototypeId);

		int mergeFailCountAfterMerge = SitesUtil
				.getMergeFailCount(layoutPrototype);
	...
	protected void selectLayoutBranch(ActionRequest actionRequest)
		...
		long layoutSetBranchId = ParamUtil.getLong(actionRequest,
				"layoutSetBranchId");

		long layoutBranchId = ParamUtil
				.getLong(actionRequest, "layoutBranchId");

		StagingUtil.setRecentLayoutBranchId(request, layoutSetBranchId,
				themeDisplay.getPlid(), layoutBranchId);
	}
	...
	protected void selectLayoutSetBranch(ActionRequest actionRequest)
		...
		LayoutSetBranch layoutSetBranch = LayoutSetBranchLocalServiceUtil
				.getLayoutSetBranch(layoutSetBranchId);

		StagingUtil.setRecentLayoutSetBranchId(request,
	...
	protected void setThemeSettingProperties(ActionRequest actionRequest,
			...
			String deviceThemeId) throws PortalException, SystemException {
	...
	protected String updateCloseRedirect(ThemeDisplay themeDisplay,
	...
	protected void updateDisplayOrder(ActionRequest actionRequest)
		...
		boolean privateLayout = ParamUtil.getBoolean(actionRequest,
				"privateLayout");
		long parentLayoutId = ParamUtil
				.getLong(actionRequest, "parentLayoutId");
	...
	protected Object[] updateLayout(ActionRequest actionRequest,
		...
		long stagingGroupId = ParamUtil
				.getLong(actionRequest, "stagingGroupId");
		boolean privateLayout = ParamUtil.getBoolean(actionRequest,
				"privateLayout");
		long layoutId = ParamUtil.getLong(actionRequest, "layoutId");
		long parentLayoutId = ParamUtil.getLong(uploadPortletRequest,
				"parentLayoutId");
		...
		Map<Locale, String> friendlyURLMap = LocalizationUtil
				.getLocalizationMap(actionRequest, "friendlyURL");
		boolean iconImage = ParamUtil.getBoolean(uploadPortletRequest,
				"iconImage");
		byte[] iconBytes = getIconBytes(uploadPortletRequest, "iconFileName");
		long layoutPrototypeId = ParamUtil.getLong(uploadPortletRequest,
				"layoutPrototypeId");
		...
		if (cmd.equals(Constants.ADD)) {
			...
			UnicodeProperties typeSettingsProperties = PropertiesParamUtil
					.getProperties(actionRequest, "TypeSettingsProperties--");

			if (inheritFromParentLayoutId && (parentLayoutId > 0)) {
				Layout parentLayout = LayoutLocalServiceUtil.getLayout(groupId,
						privateLayout, parentLayoutId);

				layout = LayoutServiceUtil.addLayout(groupId, privateLayout,
				...
				if (parentLayout.isTypePortlet()) {
					ActionUtil.copyPreferences(actionRequest, layout,
							parentLayout);

					SitesUtil.copyLookAndFeel(layout, parentLayout);
				}
			} else if (layoutPrototypeId > 0) {
				LayoutPrototype layoutPrototype = LayoutPrototypeServiceUtil
						.getLayoutPrototype(layoutPrototypeId);
				...
				if (Validator.isNotNull(layoutPrototypeLinkEnabled)) {
					serviceContext.setAttribute("layoutPrototypeLinkEnabled",
							layoutPrototypeLinkEnabled);
				}

				serviceContext.setAttribute("layoutPrototypeUuid",
						layoutPrototype.getUuid());

				layout = LayoutServiceUtil.addLayout(groupId, privateLayout,
						parentLayoutId, nameMap, titleMap, descriptionMap,
						keywordsMap, robotsMap, LayoutConstants.TYPE_PORTLET,
						typeSettingsProperties.toString(), hidden,
						friendlyURLMap, serviceContext);
			} else {
				long copyLayoutId = ParamUtil.getLong(uploadPortletRequest,
						"copyLayoutId");
				...
				if (copyLayoutId > 0) {
					try {
						copyLayout = LayoutLocalServiceUtil.getLayout(groupId,
								privateLayout, copyLayoutId);

						if (copyLayout.isTypePortlet()) {
							LayoutTypePortlet copyLayoutTypePortlet = (LayoutTypePortlet) copyLayout
									.getLayoutType();

							layoutTemplateId = copyLayoutTypePortlet
									.getLayoutTemplateId();

							typeSettingsProperties = copyLayout
				...
				layoutTypePortlet.setLayoutTemplateId(themeDisplay.getUserId(),
						layoutTemplateId);

				LayoutServiceUtil.updateLayout(groupId, privateLayout,
			...
			layout = LayoutLocalServiceUtil.getLayout(groupId, privateLayout,
					layoutId);

			oldFriendlyURL = layout.getFriendlyURL(themeDisplay.getLocale());

			layout = LayoutServiceUtil.updateLayout(groupId, privateLayout,
					layoutId, layout.getParentLayoutId(), nameMap, titleMap,
					descriptionMap, keywordsMap, robotsMap, type, hidden,
					friendlyURLMap, Boolean.valueOf(iconImage), iconBytes,
					serviceContext);

			layoutTypeSettingsProperties = layout.getTypeSettingsProperties();

			if (oldFriendlyURL.equals(layout.getFriendlyURL(themeDisplay
					.getLocale()))) {

				oldFriendlyURL = StringPool.BLANK;
			}

			UnicodeProperties formTypeSettingsProperties = PropertiesParamUtil
					.getProperties(actionRequest, "TypeSettingsProperties--");
			...
			if (type.equals(LayoutConstants.TYPE_PORTLET)) {
				...
				if ((copyLayoutId > 0)
					...
					try {
						...
						if (copyLayout.isTypePortlet()) {
							layoutTypeSettingsProperties = copyLayout
									.getTypeSettingsProperties();

							ActionUtil.copyPreferences(actionRequest, layout,
									copyLayout);

							//Start : Duplicate the webcontent along with the page
							
							LayoutTypePortlet layTypePortletTemp = (LayoutTypePortlet) layout.getLayoutType();
							List<String> portletllist = layTypePortletTemp.getPortletIds();
							...
							//End : Duplicate the webcontent along with the page
							SitesUtil.copyLookAndFeel(layout, copyLayout);
						}
					} catch (NoSuchLayoutException nsle) {
					}
					...
					layoutTypeSettingsProperties
							.putAll(formTypeSettingsProperties);

					LayoutServiceUtil.updateLayout(groupId, privateLayout,
							layoutId, layout.getTypeSettings());
				}
			} else {
				layout.setTypeSettingsProperties(formTypeSettingsProperties);

				layoutTypeSettingsProperties.putAll(layout
						.getTypeSettingsProperties());
		...
		updateLookAndFeel(actionRequest, themeDisplay.getCompanyId(),
				liveGroupId, stagingGroupId, privateLayout,
	...
	protected void updateLayoutRevision(ActionRequest actionRequest,
		...
		LayoutRevision enableLayoutRevision = LayoutRevisionLocalServiceUtil
				.updateLayoutRevision(serviceContext.getUserId(),
						layoutRevisionId, layoutRevision.getLayoutBranchId(),
		...
		if (lastLayoutRevision != null) {
			LayoutRevision newLayoutRevision = LayoutRevisionLocalServiceUtil
					.addLayoutRevision(serviceContext.getUserId(),
							layoutRevision.getLayoutSetBranchId(),
	...
	protected void updateLookAndFeel(ActionRequest actionRequest,
			long companyId, long liveGroupId, long stagingGroupId,
			boolean privateLayout, long layoutId,
			UnicodeProperties typeSettingsProperties, String themeId)
			throws Exception {

		String[] devices = StringUtil.split(ParamUtil.getString(actionRequest,
				"devices"));

		for (String device : devices) {
			String deviceThemeId = ParamUtil.getString(actionRequest, device
					+ "ThemeId");
			String deviceColorSchemeId = ParamUtil.getString(actionRequest,
					device + "ColorSchemeId");
			String deviceCss = ParamUtil.getString(actionRequest, device
					+ "Css");
			...
			if (deviceInheritLookAndFeel) {
				deviceThemeId = ThemeFactoryUtil
						.getDefaultRegularThemeId(companyId);
				...
				deviceColorSchemeId = getColorSchemeId(companyId,
						deviceThemeId, deviceColorSchemeId, deviceWapTheme);

				updateThemeSettingsProperties(actionRequest, companyId,
						typeSettingsProperties, themeId, device, deviceThemeId,
						deviceWapTheme);

				//Gaurav: Added to update the scss file on runtime, recompile and re-deploy
			//	ScssUtil.updateScssVariables(companyId, themeId,
			...
			LayoutServiceUtil.updateLayout(groupId, privateLayout, layoutId,
					typeSettingsProperties.toString());

			LayoutServiceUtil.updateLookAndFeel(groupId, privateLayout,
					layoutId, deviceThemeId, deviceColorSchemeId, deviceCss,
					deviceWapTheme);
		}
	}
	...
	protected UnicodeProperties updateThemeSettingsProperties(
		...
		Map<String, ThemeSetting> themeSettings = theme
		...
		setThemeSettingProperties(actionRequest, typeSettingsProperties,
				themeId, themeSettings, device, deviceThemeId);
