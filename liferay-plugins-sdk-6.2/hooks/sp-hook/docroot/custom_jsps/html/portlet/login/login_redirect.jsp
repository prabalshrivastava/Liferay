...
<%
	...
	boolean anonymousAccount = ParamUtil.getBoolean(request,
			"anonymousUser");
%>

<c:if test="<%=anonymousAccount && company.isStrangers()%>">
	<div class="hide lfr-message-response"
		id="<portlet:namespace />login-status-messages"></div>
	<div class="anonymous-account">
		<portlet:actionURL var="updateIncompleteUserURL">
			<portlet:param name="struts_action"
				value="/login/create_anonymous_account" />
			<portlet:param name="<%=Constants.CMD%>"
				value="<%=Constants.UPDATE%>" />
			<portlet:param name="emailAddress" value="<%=emailAddress%>" />
		</portlet:actionURL>

		<aui:form action="<%=updateIncompleteUserURL%>" method="post"
			name="fm">
			<div class="alert alert-success">
				<liferay-ui:message
					key="your-comment-has-already-been-posted.-would-you-like-to-create-an-account-with-the-provided-information" />
			</div>
			...
			<aui:button
				onClick='<%=renderResponse.getNamespace() + "closeDialog();"%>'
				value="cancel" />
		</aui:form>
	</div>
</c:if>
				...
				uri,
				{
					...
					on: {
						failure: function(event, id, obj) {
							message = '<%=UnicodeLanguageUtil.get(pageContext,
...
<aui:script use="aui-base">
	if (window.opener) {
		...
		var redirectUrl = '<%=HtmlUtil.escapeJS(themeDisplay.getURLSignIn())%>';
		
		<% if(Validator.isNotNull(ParamUtil.getString(request, "redirect"))) { %>
		 	redirectUrl = '<%=HtmlUtil.escapeJS(ParamUtil.getString(request, "redirect"))%>';
		<%}%>
		...
		if (typeof(afterLogin) == 'function') {
			...
			window.opener.parent.location.href = redirectUrl;
