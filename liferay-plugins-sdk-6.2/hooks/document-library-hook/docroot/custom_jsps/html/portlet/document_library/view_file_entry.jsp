...
boolean hasVideo = ThumbnailUtil.hasVideo(fileVersion);
			...
			<div class="body-row">
				<div class="document-info">
					<c:if test="<%= showAssetMetadata %>">
						...
						<span class="document-thumbnail">
							...
							String thumbnailSrc = ThumbnailUtil.getThumbnailUrl(fileEntry, themeDisplay);
				...
				<c:if test="<%= PropsValues.DL_FILE_ENTRY_PREVIEW_ENABLED %>">
					<div>
						...
						//	previewFileURL = ThumbnailUtil._getPreviewURL(fileEntry, fileVersion, "", false, themeDisplay.getPortalURL(), themeDisplay.getPathContext());
						}
						else if (hasVideo) {
							...
							videoThumbnailURL = ThumbnailUtil.getThumbnailUrl(fileEntry, themeDisplay.getPathThemeImages(), themeDisplay.getPortalURL(), themeDisplay.getPathContext(), 2);
						}
						...
						if (Validator.isNotNull(previewQueryString)) {
							...
							/*	if (PropsValues.DL_FILE_ENTRY_PREVIEW_VIDEO_CONTAINERS.length > 0) {
								...
								previewFileURLs = ThumbnailUtil.getVideoUrls(themeDisplay.getSiteGroupName(), fileVersion).toArray(new String[]{});
							}
							else {
								previewFileURLs = new String[1];
											...
											<div class="lfr-preview-file-content lfr-preview-image-content" id="<portlet:namespace /><%= randomNamespace %>previewFileContent">
												<div class="lfr-preview-file-image-current-column">
													<div class="lfr-preview-file-image-container">
														<img class="lfr-preview-file-image-current" src="<%= previewFileURL %>" alt="Preview Image" />
													</div>
												</div>
											</div>
										</div>
										...
										<div class="lfr-preview-file lfr-preview-video" id="<portlet:namespace /><%= randomNamespace %>previewFile">
											<div class="lfr-preview-file-content lfr-preview-video-content">
												<div class="lfr-preview-file-video-current-column">
													<div id="<portlet:namespace /><%= randomNamespace %>previewFileContent">
														<video id="videoTag"
									...
									<c:otherwise>
										<div class="lfr-preview-file" id="<portlet:namespace /><%= randomNamespace %>previewFile">
											<div class="lfr-preview-file-content" id="<portlet:namespace /><%= randomNamespace %>previewFileContent">
												<div class="lfr-preview-file-image-current-column">
													<div class="lfr-preview-file-image-container">
														<img class="lfr-preview-file-image-current" id="<portlet:namespace /><%= randomNamespace %>previewFileImage" src="<%= previewFileURL + "1" %>"  alt="Preview Image"/>
													</div>
