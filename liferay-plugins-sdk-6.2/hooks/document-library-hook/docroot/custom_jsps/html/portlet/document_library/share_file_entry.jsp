...
<%@ page import="com.sambaash.platform.srv.sharing.model.SPSharing" %>
<%@ page import="com.sambaash.platform.srv.sharing.service.SPSharingLocalServiceUtil" %>
...
String showFileSharing = "false";
String emailDomain = "noFiltering";
try {
	spParameters = SPParameterLocalServiceUtil.findBySPParameterGroupIdAndName(scopeGroupId, "show.internal.filesharing");
	showFileSharing = spParameters.getValue();
	spParameters = SPParameterLocalServiceUtil.findBySPParameterGroupIdAndName(scopeGroupId, "internal.emailDomain");
	emailDomain = spParameters.getValue();
}catch(Exception e){
...
<div id="specialBox">
 ...
 <b>  <%=sharedFileName %> <%=LabelUtil.getLabel(pageContext, themeDisplay,"label.shared.successfully")%></b></div> 
  <div id="delete_message" class="success_message" style="display: inline-table; margin-left: 20px; margin-top: 30px;">
 <b> <%=LabelUtil.getLabel(pageContext, themeDisplay,"label.file.folder.sharing.removed.successfully")%></b></div> 
 
</div>
<div id="share_details">
<div class = "sharePageTitle" style="font-weight:bold;font-size:2.5em"><%=sharedFileName %></div>
<%if(showFileSharing.equalsIgnoreCase("true")){ %>
<div class="user_item">
<%}else{ %>
<div class="user_item hide">
<%} %>
 	<div class="user_item_Bio">
        <ul class="accor_list accor_list_bio">
           <li id="accor_list1" class="accor_list_item accor_list_icon1">
               <a href="#"><%=LabelUtil.getLabel(pageContext, themeDisplay,"label.internal.share")%> <span class="shareInlineText" style=""><%=LabelUtil.getLabel(pageContext, themeDisplay,"label.add.menarini.users.only")%></span></a>
 	...
 	<div class="user_item_Bio">
        <ul class="accor_list accor_list_bio"> 
           <li id="accor_list2" class="accor_list_item accor_list_icon2">
               <a href="#"><%=LabelUtil.getLabel(pageContext, themeDisplay,"label.external.share")%></a>
                   <div class="accor_subContent accor_subContent_Bio">
...
<div class="sharedFiles_list">
		...
		<tbody id ="sharedFiles-table-body">
				<tr>
					<td class="mailtemplate-list-title" width="25%">
						<b><%=LabelUtil.getLabel(pageContext, themeDisplay,"label.file.folder.name")%></b>
					</td>
					<td class="mailtemplate-list-title" width="34%">
						<b><%=LabelUtil.getLabel(pageContext, themeDisplay,"label.shared.to")%></b>
					</td>
					<td class="mailtemplate-list-title" width="19%">
						<b><%=LabelUtil.getLabel(pageContext, themeDisplay,"label.duration")%></b>
					</td>
					<td class="mailtemplate-list-title" width="22%">
						<b><%=LabelUtil.getLabel(pageContext, themeDisplay,"label.action")%></b>
					</td>
					...
					<tr>
						<td class="template-list" style="padding: 10px">
							...
							<c:if test="${sharing.internalShare == 'false' }">
								<img src="<%= themeDisplay.getPathThemeImages()%>/common/externalShare.png" width="20px" title="<%=LabelUtil.getLabel(pageContext, themeDisplay,"label.external.share")%>" alt="<%=LabelUtil.getLabel(pageContext, themeDisplay,"label.external.share")%>"/>
							</c:if>
						</td>
						<td class="template-list" style="padding: 10px">
						...
						<td class="template-list-actions">
						    ...
						    <input type="checkbox" <c:if test="${sharing.writeAccess == 'Enabled' }">checked</c:if> 
						    	...
						    	onchange="changeWriteAccess(this.id, '<portlet:resourceURL />')" ><%=LabelUtil.getLabel(pageContext, themeDisplay,"label.write.access")%></input>
						   	</div>
						    <c:if test="${sharing.showExtend }">
								<a href="javascript:extendEndDate('${sharing.index}')" class="ext_icon" id="extendDate_${sharing.index}" ><img src="<%=themeDisplay.getPathThemeImages()%>/common/revision2.png" title="<%=LabelUtil.getLabel(pageContext, themeDisplay,"label.extend")%>" alt="<%=LabelUtil.getLabel(pageContext, themeDisplay,"label.remove")%>"></a>
							...
							<a href="javascript:deleteSharedInfo('${sharing.index}')" class="remove_icon" id="remove_${sharing.index}"><img src="<%=themeDisplay.getPathThemeImages()%>/common/delete2.png" title="<%=LabelUtil.getLabel(pageContext, themeDisplay,"label.remove")%>" alt="<%=LabelUtil.getLabel(pageContext, themeDisplay,"label.remove")%>"></a>  
						</td>
					</tr>
					</c:forEach>
				</c:if>
				<c:if test="${empty sharingList}">
					<tr id="emptyMsgRow">
						<td class="template-list" style="padding: 10px" id="emptyTable_msg">
							<b><%=LabelUtil.getLabel(pageContext, themeDisplay,"label.file.folder.not.shared")%> </b></td>
					</tr>
				</c:if>
				</tbody>
	</table>
 ...
 function deleteSharedInfo(k) {
	//	try {
			...
			var action="Delete";
			var dataObj = {};
			dataObj[ pns  + "sharingId" ] =  sharingId;
			dataObj[ pns + "sharedToUser"] = sharedToUser;
			dataObj[ pns + "action"] = action;
						var A = AUI();
											...
											on : {
												success : function() {
													...
													if(x == 1){
														...
														emptyMsgDiv.innerHTML = "<b>" + "<%=LabelUtil.getLabel(pageContext, themeDisplay,"label.file.folder.not.shared")%>" + "</b>";
 ...
 function extendDateForSharing(k) {
		//	try {
				...
				var dataObj = {};
				dataObj[ pns + "extendDuration"] = extendDuration;
				dataObj[ pns + "sharingId"] = sharingId;
				dataObj[ pns + "action"] = action;
