				      ...
				      if (_.isEmpty(data)) {
				        console.log("error");
				    	  ...
				    	  for(var i = 0; i < data.length; i++){
				    		  selectsubjects.options[selectsubjects.options.length] = new Option(data[i].SubjectTitle, data[i].SubjectCode, false, false);
				    	  }
				      }
				    },
				failure : function() {
...
function jsFilterAllCandidates(){
	var jsonfilterparameter = {"limit":1000,"modelName":modelName,"page":(pageRequested - 1),"formType":modelName};
...
function exportStorage1(e,d){
	...
	ajaxCall('GET','exportRow',exportstorageurl,{"formType":modelName,"formStorageId": encodeURIComponent(a[0])},
			function() {
