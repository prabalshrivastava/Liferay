...
public class SPERPECModelHandler {
	...
	public static void exportData(ResourceRequest resourceRequest, String response2, String data,
		...
		try {
			...
			for (Integer index = 0; index < content.length(); index++) {
				...
				for (int i = 0; i < contentJsonTitles.length(); i++) {
					...
					orderFromJSON.put(contentJsonTitles.getString(i), ((contentJson.has(contentJsonTitles.getString(i)))
	...
	private static int writeExcelEntityRow(XSSFSheet sheet, int rowNum, org.json.JSONObject entityJson, String[] order,
			int key) {
