		...
		<aui:nav cssClass="nav-list well">
			...
			<c:choose>
				<c:when test='<%= (((folderId == rootFolderId) && !expandFolder) || ((folder != null) && (folder.isRoot() && !folder.isDefaultRepository() && !expandFolder))) && !browseBy.equals("file-entry-type") %>'>
					...
					<%
						if(layout.isPublicLayout() && GetterUtil.getBoolean(com.liferay.portal.kernel.util.PropsUtil.get("dl.show.private.docs.link"))){
					%>
							
							<li class="app-view-navigation-entry"> 
								<a class="browse-folder file-entry-type" href='<%=themeDisplay.getPortalURL() + "/user" + "/" + themeDisplay.getUser().getScreenName() + "/home" %>' target = 'top'  id="" role="menuitem" title=""> 
									<i class="icon-lock"></i> <span class="nav-item-label"> Personal Documents </span> 
								</a> 
							</li>
							<%-- <a href='<%=themeDisplay.getPortalURL() + "/user" + "/" + themeDisplay.getUser().getScreenName() + "/home" %>' target = 'top' ></a> --%>
					<%		
						}
					%>
