...
function changeWriteAccess(id, ajaxUrl) {
	 id = id.substring(id.indexOf('_') + 1)
	 
	 try {
	 AUI().use('aui-io-request', 'aui-node' , function(A) {
		 
		 var change = A.one("#writeAccess_" + id).get('checked');
		 var dataObj = {};
		 // pns is declared in share_file_entry.jsp
		 dataObj[ pns + "action"] = 'changePermission';
		 dataObj[ pns + "sharingId"] = id;
		 dataObj[ pns + "writeAccess"] = change;
		...
		alert('Error occurred while updating the status -> ' + err); 
	 }
}