...
public class Netlogon {
	...
	public NtlmUserAccount logon(
		...
		_log.debug("NetLogon logon start");
		...
		try {
			...
			_log.debug("NetLogon logon connection established");
			...
			_log.debug("NetLogon logon domain= " + domain + "userName = " + userName + " workstation= " +workstation );
			...
			_log.debug("netlogonNetworkInfo " +  netlogonNetworkInfo);
			...
			_log.debug("netrLogonSamLogon " +  netrLogonSamLogon);
			

			DcerpcHandle dcerpcHandle = netlogonConnection.getDcerpcHandle();

			_log.debug("NetLogon logon autenticating ");
			dcerpcHandle.sendrecv(netrLogonSamLogon);
			_log.debug("NetLogon logon netrLogonSamLogon.getStatus() = " + netrLogonSamLogon.getStatus());
			...
			if (netrLogonSamLogon.getStatus() == 0) {
				...
				_log.debug("Status 0, user authenticated. netlogonValidationSamInfo " + netlogonValidationSamInfo);
