...
public class FreeMarkerTemplateContextHelper extends TemplateContextHelper {
	...
	public void prepare(Template template, HttpServletRequest request) {
		...
		// Sambaash variables
		addCustomProperties(template, request, themeDisplay);
	}
	
	private void addCustomProperties(Template template,
			HttpServletRequest request, ThemeDisplay themeDisplay) {
		try {
			template.put("hasAccess", SambaashUtil.isAdmin(
					themeDisplay.getScopeGroupId(), themeDisplay.getUserId()));
			template.put("dashboardUrl", SambaashUtil.getDashboardUrl(
					themeDisplay.getScopeGroupId(), themeDisplay.getUserId()));
		} catch (Exception e) {
			e.printStackTrace();
		}
