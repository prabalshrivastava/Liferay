			...
			'<tpl for=".">',
				'<tpl if="!values.error">',
					'<li class="upload-file {[ values.temp ? "upload-complete pending-file selectable" : "" ]} {[ values.selected ? "selected" : "" ]}" data-fileId="{id}" data-fileName="{[ Liferay.Util.escapeHTML(values.name) ]}" data-title="{[ Liferay.Util.escapeHTML(values.title ? values.title : values.name) ]}" id="{id}">',
						'<input class="{[ !values.temp ? "hide" : "" ]} select-file" data-fileName="{[ Liferay.Util.escapeHTML(values.fullPath ? values.fullPath  : values.name) ]}" data-title="{[ Liferay.Util.escapeHTML(values.title ? values.title : values.name) ]}" id="{id}checkbox" name="{$ns}selectUploadedFileCheckbox" type="{[ this.multipleFiles ? "checkbox" : "hidden" ]}" value="{[ Liferay.Util.escapeHTML(values.name) ]}" />',
						...
						'<div style="font-size:10px;word-wrap:break-all;font-weight:bold">{[ Liferay.Util.escapeHTML(values.fullPath ? ( (values.fullPath.indexOf("/") != values.fullPath.lastIndexOf("/") ) ? values.fullPath.substring(0,values.fullPath.lastIndexOf("/")) : "")  : "") ]}</div>',
		...
		var Upload = A.Component.create(
			{
				ATTRS: {
					...
					hook_beforeStartupload: {
						value: null
					},
					hook_beforeFileUploadProcess: {
						value: null
					},
					hook_onDeleteClick :{
						value: null
				...
				prototype: {
					...
					renderUI: function() {
						...
						A.FileHTML5.prototype.startUploadOriginal = A.FileHTML5.prototype.startUpload;
						A.FileHTML5.prototype.startUpload = function(url, parameters, fileFieldName){
							parameters = parameters || {};
							parameters.fullPath = this.fullPath;
							// hook - useful to add any parameters dynamicall
							console.log(instance.get('hook_beforeStartupload'));
							if(instance.get('hook_beforeStartupload')){
								var bsuHook = instance.get('hook_beforeStartupload');
								bsuHook(instance,this, parameters);
							}
							console.log("in startupload override method" + parameters.fullPath);
							A.FileHTML5.prototype.startUploadOriginal.call(this,url, parameters, fileFieldName);
						}
					},
					...
					_formatTempFiles: function(fileNames) {
						...
						//TODO: Below code just lists the files under temp folder but files under sub folders of temp also be listed.
						// Commenting for now, so that nothing will be shown to user though they are temp files. If time permits have to list all the files
						...
						if (fileNames.length && Lang.isArray(fileNames)) {
							...
							var files = AArray.map(
								...
								function(item, index, collection) {
									...
									item = item.replace('com.liferay.portlet.documentlibrary.action.EditFileEntryAction','');
					...
					_getValidFiles: function(data) {
						...
						var minImageHeight = instance.get('minImageHeight');
						var minImageWidth = instance.get('minImageWidth');
						var maxImageHeight = instance.get('maxImageHeight');
						var maxImageWidth = instance.get('maxImageWidth');
					...
					_handleDrop: function(event) {
						...
						var dataTransferItems = event._event.dataTransfer.items;
						if(dataTransferItems){
							// To handle the directory
							if (dataTransferItems && (target === uploaderBoundingBox || uploaderBoundingBox.contains(target))) {
								
								var onError = function(e){
									alert(e.code);
								}
								
								var handleFileUpload = function(fileItem){
									fileItem.file(function(file){
										var auiFile = new A.FileHTML5(file);
										
										auiFile.fullPath = fileItem.fullPath;
										event.fileList = [auiFile];
										console.log("File Size " + (file.size || file.fileSize));
										console.log("File full path " + (auiFile.fullPath));
								...
								/** Reading directory contentes */
								var readDirectory = function (dirEntry, callback) {
									  var dirReader = dirEntry.createReader();
									  var entries = [];

									  // Call the reader.readEntries() until no more results are returned. Api may not return all files in single call. so have to recursively call until length is zero
									  var readEntries = function() {
									     dirReader.readEntries (function(results) {
									    	 console.log("reading " + dirEntry.fullPath );
									    	 console.log("inside readentries length" + results.length);
									      if (!results.length) {
									        callback(entries);
									      } else {
									        entries = entries.concat(AArray(results));
									        readEntries();
									      }
									    }, onError);
									  };
								...
								/** End reading directory contents */	

								var callbackfordir;
								/** Call back for directory reader. Once directory reading completes, this callback method will be invoked */
								callbackfordir = function(entries){
									AArray.each(entries,function(entry,inde,entries){
										if(entry.isFile){
											handleFileUpload(entry);
										} 
									});
									AArray.each(entries,function(entry,inde,entries){
										if(entry.isDirectory){
											readDirectory(entry,callbackfordir);
										}
									});
								};
								/** End callback - directory reading */
								
								
								/** loop through all draaged items */
								for(var i = 0 ; i < dataTransferItems.length; i++){
									var item = dataTransferItems[i].webkitGetAsEntry();
									console.log("looping dt items  " + item.fullPath);
									if(item.isDirectory){
										readDirectory(item,callbackfordir);
									}
									if(item.isFile){
					...
					_onDeleteFileClick: function(currentTarget) {
						...
						var hook_onDeleteClick = instance.get('hook_onDeleteClick');
						var data = instance.ns(
								{
									fileName: li.attr('data-fileName')
								});
						if(hook_onDeleteClick){
							hook_onDeleteClick(instance,data);
						}
					...
					_onFileSelect: function(event) {
						...
						//hook
						var beforeFileUploadProcess  = instance.get("hook_beforeFileUploadProcess");
						
						if(beforeFileUploadProcess){
							fileList = beforeFileUploadProcess(instance,event,event.fileList);
						}
