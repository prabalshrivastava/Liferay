...
public class EditUserAction extends PortletAction {
	...
	public void processAction(
		...
		try {
			...
			boolean isCustomProfileUrl = false;
			String parameter = SambaashUtil.getParameter("control.panel.link.profile", themeDisplay.getScopeGroupId());
			if(Validator.isNotNull(parameter)){
				if(parameter.equalsIgnoreCase("true")){
					isCustomProfileUrl = true;
				}
			}	
			if(isCustomProfileUrl){
				sendRedirect(actionRequest, actionResponse, redirect);
			}else{
				if(Validator.isNotNull(user)){
					actionResponse.sendRedirect(themeDisplay.getPortalURL()+"/"+user.getScreenName());
				}else{
					sendRedirect(actionRequest, actionResponse, redirect);
				}
			}
	...
	public ActionForward render(
		...
		throws Exception {
		try {
			User usr = PortalUtil.getSelectedUser(renderRequest);
		}
		catch (Exception e) {
	...
	protected User addUser(ActionRequest actionRequest) throws Exception {
		...
		String parameter =  SambaashUtil.getParameter("admin.create.user.sendemail", themeDisplay.getScopeGroupId());
		if(Validator.isNotNull(parameter)){
			if(parameter.equalsIgnoreCase("true")){
				sendEmail = true;
			}
		}	
		_log.error("SendEmail enabled " + sendEmail);
	...
	private static Log _log = LogFactoryUtil.getLog(com.liferay.portlet.usersadmin.action.EditUserAction.class);
}