...
public class PortalLDAPImporterImpl implements PortalLDAPImporter {
	...
	protected User importUser(
		...
		try {
			...
			if (user == null) {
				...
				// START-March 29, 2016 - Added by Naresh //TODO: Check if addUser event availble in the system, if so move the code to event listener.
				String mail = LDAPUtil.getAttributeString(attributes, SambaashConstants.LDAP_MAIL);
				mapAllSharingsForUser(user,mail);
				// END -
				isNew = true;
			}
	...
	/**
	 * Applicable for users in ldap
	 * This will update SpSharing table, userId column to actual userId.
	 * If the user receives file/folder sharings even before his account got created, then userId column in spshairng contains 0 (zero).// check setUserId(0) in ShareFileAndFolderAction
	 * So this userId column has to updated upon user account got created.
	 * 
	 *  In ldap, user can have 2 email id's. Hence user can receive sharings to either of 2 email id's. So have to update records in sharing table with both email ids
	 * 
	 * 
	 * @param user
	 * @param mail
	 */
	private void mapAllSharingsForUser(User user,String mail) {
		try {
			List<SPSharing> userSharings = new ArrayList<SPSharing>();
			userSharings.addAll(SPSharingLocalServiceUtil
					.getUserSharingByEmail(user.getEmailAddress()));
			userSharings.addAll(SPSharingLocalServiceUtil
					.getUserSharingByEmail(mail));
			for (SPSharing sharing : userSharings) {
				_log.error("sharing.getUserId() :" + sharing.getUserId() + " using " + user.getUserId());
				if (Long.valueOf(sharing.getUserId()).equals(0L)) {
					sharing.setUserId(user.getUserId());
					SPSharingLocalServiceUtil.updateSPSharing(sharing);
				}
			}
		} catch (Exception e) {
			_log.error("error while mapping sharings to user", e);
		}
	}
