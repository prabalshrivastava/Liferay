...
<div id="<portlet:namespace />inviteFriendsEmailContent">
	<form id="<portlet:namespace />ext_form" action="<%=inviteFriendsURL %>" method="POST" onsubmit="externalShare(event)" class="s1">
		...
		<div class="fieldset">
			<div><label for="ip_to" class="sp-group-column sp-group-of-ten" style="vertical-align: middle;"><span class="sp-group-ui-db"><%=LabelUtil.getLabel(pageContext, themeDisplay,"label.shareto")%></span></label><div class="sp-group-nonuple-column sp-group-column sp-group-of-ten"><div data-autocomplete-dom-id="ext-sis-holder" data-group-id="<%=folderId %>" class="ip-to">
				<ul data-autocomplete-dom-id="ext-selected-items" class="horizontal sp-group-ui-dib sp-group-ui-vam">
					
				</ul>
				<span class="ip-to-input-outer"><span style="visibility: hidden; z-index: -10000; position: relative; width: 100%;"><%=LabelUtil.getLabel(pageContext, themeDisplay,"label.add.email.address")%>&nbsp;</span><input data-autocomplete-dom-id="ext-input" id="ext_to" type="text" tabindex="0" placeholder="<%=LabelUtil.getLabel(pageContext, themeDisplay,"label.add.email.address")%>" autocomplete="off" value="" class="ip-to-input" /></span></div>
		...
		<div style="text-align:center;">
			<span class="sp-group-column sp-group-of-ten"></span><input id="ext_submit" type="submit" value="<%=LabelUtil.getLabel(pageContext, themeDisplay,"label.external.share")%>" tabindex="3" class="sp-group-column" />
		</div>
	</form>
</div>
	...
	*/
	...
	try{
        ...
        if(send_to.length == 0) {
			...
			temp_url += "&<portlet:namespace />folderId=" + folderId + "&<portlet:namespace />fileEntryId=" + file_entry_id + "&<portlet:namespace />send_to=" + send_to +"&<portlet:namespace />invite_by=" + invite_by+"&<portlet:namespace />shareType=0&<portlet:namespace />durationList=" + durationList + "&<portlet:namespace />permissionList=" + rwList;
