...
public class NtlmManager {
	...
	public NtlmManager(
		...
		_log.debug("Creating ntml manager. Inside NtlmManager constructor");
		_log.debug("domain=" + domain + " domainController="+domainController + " domainControllerName= " + domainControllerName + " serviceAccount=" + serviceAccount + " servicePassword=" + servicePassword) ;
		setConfiguration(
	...
	public NtlmUserAccount authenticate(byte[] material, byte[] serverChallenge)
		...
		_log.debug("Before authenication");

		Type3Message type3Message = new Type3Message(material);

		_log.debug("_NTLMSSP_NEGOTIATE_EXTENDED_SESSION_SECURITY =" + type3Message.getFlag(
		...
		if (type3Message.getFlag(
			...
			_log.debug(" _NTLMSSP_NEGOTIATE_EXTENDED_SESSION_SECURITY = " + true + " and getNTResponse().length == 24");
			...
			_log.debug(" _NTLMSSP_NEGOTIATE_EXTENDED_SESSION_SECURITY = " + false + " or getNTResponse().length != 24");	
		}
