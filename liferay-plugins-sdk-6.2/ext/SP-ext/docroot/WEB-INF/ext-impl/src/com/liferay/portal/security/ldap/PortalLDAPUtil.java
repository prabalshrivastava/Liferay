...
public class PortalLDAPUtil {
	...
	/**
	 * March 22, 2016 -Naresh  getUserByScreenName, getUserByEmailAddress and getUser methods added
	 * 
	 * @param ldapServerId
	 * @param companyId
	 * @param screenName
	 ...
	 * @throws Exception
	 */
	public static Binding getUserByScreenName(
			long ldapServerId, long companyId, String screenName)
					throws Exception {
		
		return getUser(
				ldapServerId, companyId, screenName,true);
	}
	public static Binding getUserByEmailAddress(
			long ldapServerId, long companyId, String emailAddress)
					throws Exception {
		
		return getUser(
				ldapServerId, companyId, emailAddress,false);
	}
	
	// March 22, 2016 -Naresh - added below method
	// accountName : indicates screenName when byScreenName is true else indicates emailAddress
	public static Binding getUser(
			long ldapServerId, long companyId, String accountName,boolean byScreenName)
					throws Exception {
		_log.debug("getUser By screenName " + byScreenName + " screenName/EmailAddress " + accountName);
		String postfix = LDAPSettingsUtil.getPropertyPostfix(ldapServerId);
		...
		try {
			...
			_log.debug("Search query " + sb.toString());
	...
	public static Attributes getUserAttributes(
		...
		// Naresh : 13th Jan 2015 Added below code to retrieve the extra attributes. Done as Part of Menarini ESN 
		//displayName,company,division,title,mail,employeeID,managertelephoneNumber,mobile,facsimileTelephoneNumber
		String[] extraAttributes = { SambaashConstants.LDAP_DEPARTMENT,SambaashConstants.LDAP_COMPANY,SambaashConstants.LDAP_COUNTRY,SambaashConstants.LDAP_DISPLAYNAME,
				SambaashConstants.LDAP_DIVISION,SambaashConstants.LDAP_TITLE,SambaashConstants.LDAP_MAIL,SambaashConstants.LDAP_EMPLOYEE_ID,
				SambaashConstants.LDAP_MANAGER,SambaashConstants.LDAP_TELEPHONE_NUMBER,SambaashConstants.LDAP_MOBILE,SambaashConstants.LDAP_FACSIMILE_TELEPHONE_NUMBER
				}; 
		String[] allAttributes = new String[mappedUserAttributeIds.length + extraAttributes.length];
		System.arraycopy(
				mappedUserAttributeIds, 0, allAttributes, 0, mappedUserAttributeIds.length);
		System.arraycopy(
					extraAttributes, 0, allAttributes, mappedUserAttributeIds.length,
					extraAttributes.length);


		Attributes attributes = _getAttributes(
			ldapContext, fullDistinguishedName, allAttributes);

		if (_log.isDebugEnabled()) {
			for (String attributeId : allAttributes) {
				Attribute attribute = attributes.get(attributeId);

				if (attribute == null) {
					_log.debug("LDAP user attribute is null for" +  attributeId);
					continue;
				}
