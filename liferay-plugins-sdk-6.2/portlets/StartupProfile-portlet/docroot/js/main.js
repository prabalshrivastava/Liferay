...
function initialize(errorMessage1,profileSaveMessage,successMessage,profileError,errorMessage,errorMessage2,errorMessage3,deleteConfirmMsg,removeSuccessMsg,removeFailedMsg,removeFailRefreshMsg,thankuMsg,startupFailMsg,viewBrief,goToProfile,okLabel,ajaxUrl, isAddStartup, notAnimated) {
	...
	orgUEN = namespace + "organization_uen";
...
function checkOrgUEN(erromessage1,errormessage2){
...
function checkEmailAddress(erromessage1,errormessage2){
	...
	A.io.request(ajax,{
		...
		on : {
			...
			failure : function() {
				if(isActiveUser()) {
					displayMessage(erromessage1);
				} else {
					displayMessage(erromessage2);
					window.location.href= "/signin";
				}
			}
		}
...
function addSection(deleteConfirmMsg,removeSuccessMsg,removeFailedMsg,removeFailRefreshMsg,successMessage,errorMessage,errorMessage3,fieldName) {
	AUI().use('aui-node','aui-base','aui-io-request-deprecated',function(A){
							...
							A.io.request(ajax,
									{
										...
										on : {
											...
											failure : function() {
												if(isActiveUser()) {
													displayMessage(removeFailRefreshMsg);
												} else {
													displayMessage(errorMessage3);
													window.location.href= "/signin";
												}
		...
		var addBtnDup = dup.getDOMNode().parentElement.parentElement.getElementsByClassName("addButton");
		if(addBtnDup.length > 0)
			
			addBtnDup[0].onclick = function(){
	...
	function(A) {
		...
		ele.each(function(){
			...
			try {
				...
				name = name.replace("_", ".");
				var data = eval(name);
				if(this.get("type") == "checkbox" && data == true){
					document.getElementsByName(this.get("name"))[0].checked = true;
				...
				if (name==='organization.approvalDate') {
					data = eval('organization.approvalDate');
					if(data != ""){
						data = getFormattedDate(new Date(organization.approvalDate),"DD/MM/YYYY");
					}
		...
		if(testform.hasErrors()){
		 return false;
			...
			ele.each(function(){
				var name = this.get("name");
				name = name.substring(len);
				if(name != "organization_approvalDate"){
					if(this.get("type") == "checkbox"){
						if(this.getDOMNode().checked){
							obj[name] = "true";
						}else{
							obj[name] = "false";
						}
			...
			obj['documentsToDelete'] = JSON.stringify(documentsToDelete);
							...
							A.io.request(ajax,
									{
										...
										on : {
											...
											failure : function() {
												if(isActiveUser()) {
													displayMessage(removeFailrefresh);
												} else {
													displayMessage(sessionTimeOut);
													window.location.href= "/signin";
												}
											}
										}
